 ...
 *          フォールトインジェクションのアクセサ群。
 *
 * \details `test/stub/` 配下の構成規則は `test/stub/Bsw/CanIf/Wrap_ComIf.h`
...
/** `FailFromCallCount_Com_Xxx` の「無効（常にパススルー）」を表す番兵値。
...
extern uint32 CallCount_Com_IpduGroupStart;
extern uint32 CallCount_Com_IpduGroupStop;
extern uint32 CallCount_Com_EnableReceptionDM;
extern uint32 CallCount_Com_DisableReceptionDM;
extern uint32 CallCount_Com_GetStatus;
extern uint32 CallCount_Com_GetVersionInfo;
extern uint32 CallCount_Com_SendSignal;
extern uint32 CallCount_Com_ReceiveSignal;
 ...
 * ---------------------------------------------------------------------- */
/** `Wrap_Com_FAIL_FROM_CALL_COUNT_DISABLED`（既定）: 常に対応する
