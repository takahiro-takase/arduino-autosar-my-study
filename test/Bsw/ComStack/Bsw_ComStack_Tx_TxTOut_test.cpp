/**
 * \file    Bsw_ComStack_Tx_TxTOut_test.cpp
 * \brief   README.md「Tx 処理（Com → PduR → CanIf → Can の順）」コールチェーンの
 *          単体テスト（GoogleTest / CMake native_chain_tests）。TxTOut シナリオ専用。
 *
 ...
 *          API・粒度であるため、後者を Bsw_ComStack_Tx_TxTOutGroupLevel_test.cpp
...
class Bsw_ComStack_Tx_TxTOut_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_TxTOut_Test, TxTOut_OK_FiresAfterFirstTimeoutWhenArmedAndUnconfirmed)
{
...
TEST_F(Bsw_ComStack_Tx_TxTOut_Test, TxTOut_OK_RepeatsDoNotRestartOrExtendDeadline)
{
...
TEST_F(Bsw_ComStack_Tx_TxTOut_Test, TxTOut_OK_ConfirmationBeforeDeadlineCancelsIt)
{
...
TEST_F(Bsw_ComStack_Tx_TxTOut_Test, TxTOut_OK_UsesSteadyTimeoutAfterFirstConfirmedCycle)
{
