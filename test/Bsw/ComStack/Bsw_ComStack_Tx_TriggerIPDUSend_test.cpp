...
// 切り替えられるトグル式（Bsw_ComStack_Rx_test.cpp の TestRxIpduCallout と対称）。
...
class Bsw_ComStack_Tx_TriggerIPDUSend_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSend_Test, TriggerIPDUSend_OK_ForcesDispatchWithoutValueChange)
{
...
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSend_Test, TriggerIPDUSend_NG_UnknownPduIdReturnsError)
{
    /* 実行 (Act) + 評価 (Assert) */
    EXPECT_EQ(Com_TriggerIPDUSend(99U), E_NOT_OK);
}

TEST_F(Bsw_ComStack_Tx_TriggerIPDUSend_Test, TriggerIPDUSend_NG_StoppedIpduReturnsErrorWithoutTriggering)
{
...
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSend_Test, TriggerIPDUSend_OK_RespectsMinDelayTimeAndDispatchesOnceElapsed)
{
    /* 準備 (Arrange): kTestErrGroupIPdu（IPduId=3）は IpduGroupId=
     * kTestStoppableGroupId を持つため、Com_Init() 直後は既定で停止状態
...
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSend_Test, TriggerIPDUSend_OK_DoesNotConsumeNumberOfRepetitionsBudget)
{
...
// Bsw_ComStack_Tx_TriggerIPDUSend_Test（上記）は TX I-PDU 4 本（COM_TX_IPDU_MAX と同数）を
...
// そのため Bsw_ComStack_RxTimeout_test.cpp の `rx_ipdu_group` 名前空間と同じ
...
class Bsw_ComStack_Tx_TriggerIPDUSendPeriodic_Test : public IsolatedComTxFixtureBase
{
protected:
    const Com_ConfigType* GetComConfig() const override { return &kTestComConfig; }
};
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSendPeriodic_Test, TriggerIPDUSend_OK_FiresBetweenPeriodsOnceMdtElapses)
{
...
// 2026-09-21、Bsw_ComStack_Tx_ComMainFunction_test.cpp から移設
...
// 本ファイルの命名規則に合わせて変更した。
TEST_F(Bsw_ComStack_Tx_TriggerIPDUSendPeriodic_Test, TriggerIPDUSend_NG_DoesNotFireBeforePeriodElapsedWithoutTrigger)
{
