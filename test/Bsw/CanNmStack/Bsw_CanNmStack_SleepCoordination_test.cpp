...
/** テスト6専用: CANNM_CANIF_TX_PDU_ID（値2、CanNm_Cfg.h）だけを有効化した TX PDU
 *  設定。「CanNm の再送信が実際に Can_Hw_Send() まで到達したか」を
...
protected:
    ...
    /** ComM_USER_0 に FULL_COM を要求し、CanNm が Repeat Message State を抜けて
    ...
    /** NO_COM を要求し、CanNm が Prepare Bus-Sleep Mode（チャネルは SILENT_COM）
    ...
    void ArrangeSilentComAtPrepareBusSleep()
    {
        ...
        DriveNmUntil(CANNM_STATE_PREPARE_BUS_SLEEP);
    ...
    {
        uint8 rxData[CANNM_DLC] = { 0x00U, 0x02U };
        PduInfoType rxPdu = { rxData, CANNM_DLC };
        CanNm_RxIndication(0U, &rxPdu);
    }

    /** CanNm_MainFunction() を呼びながら millis を CANNM_CYCLE_MS ずつ進め、CanNm が
     ...
     *  相当を CANNM_CYCLE_MS=1000ms 刻みで辿るには最低7ティック必要なため、
     *  既定 15 で十分な余裕を持たせている）。 */
    static void DriveNmUntil(CanNm_StateType target, int maxTicks = 15)
    {
        CanNm_StateType state;
        CanNm_ModeType  mode;
 ...
 *          CanNm 協調スリープ待ち中に Bus-Off が発生し（ComM_ChannelMode が
 ...
 *
 *          本プロジェクトの同期的な設計では、CanNm が独自に Bus-Sleep Mode へ
 ...
 *          回帰テスト、最重要）:
 *          Bus-Off 中に CanNm が Prepare Bus-Sleep Mode へ独立して到達し
 *          （Stage 1 は SILENT_COM 済みのため no-op）、さらに Bus-Off 中に
 *          他ノード RX で CanNm がローカルに自律復帰した場合
...
TEST_F(Bsw_SleepCoordination_Test, BusOffDuringRxCancelledPrepareBusSleep_OK_ConvergesToFullCom)
{
    ...
    /* CanNm は Bus-Off とは無関係に独立して Prepare Bus-Sleep Mode へ到達する。
 ...
 *
 *          CanNm 協調スリープ経由（ArrangeSilentComAtPrepareBusSleep()）ではなく
...
TEST_F(Bsw_SleepCoordination_Test, BusOffDuringSilentCom_OK_ConvergesToNoComWhenNmReachesBusSleepDuringOutage)
{
    ...
    /* CanNm は Bus-Off とは無関係に独立して Bus-Sleep Mode へ到達する。この
