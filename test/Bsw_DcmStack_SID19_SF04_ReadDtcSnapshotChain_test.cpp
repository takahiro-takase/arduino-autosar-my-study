/**
 * \file    Bsw_DcmStack_SID19_SF04_ReadDtcSnapshotChain_test.cpp
 * \brief   UDS SID 0x19/0x04 reportDTCSnapshotRecordByDTCNumber の、物理層
 ...
 *          マルチフレームの型を適用する。応答は18バイト
 *          （[0x59, 0x04, DTC_H, DTC_M, DTC_L, status, recordNumber,
 *          didCount, DID×3 + 対応データ]）で、FF（6バイト）+ CF 2本
 *          （残り12バイトを7バイトずつ、ceil(12/7)=2）という、
 *          DcmStack シリーズの中で最もフレーム数の多いマルチフレーム構成
 *          になる。
 *
 *          リクエストは [0x19, 0x04, DTC_H, DTC_M, DTC_L, recordNumber]
 *          （6バイト、Single Frame に収まる）。DTC は
 *          DEM_EVENT_ENGINE_OVERHEAT（DTC=0x000101）を
 *          Dem_SetEventStatus() で FAILED 確定させ FreezeFrame を記録させた
 *          ものを使う（Bsw_Dcm_ReadDtcInfo_test.cpp と同じ手段、新規 Wrap
 *          関数は不要）。
 */
...
class Bsw_DcmStack_SID19_SF04_ReadDtcSnapshotChain_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_DcmStack_SID19_SF04_ReadDtcSnapshotChain_Test,
       ReadDtcSnapshot_OK_MultiFrameResponseReassemblesToExpectedPayloadOnCanHw)
{
     ...
     * [0x19, 0x04, 0x00,0x01,0x01, recordNumber=0x01] を 0x7E0 の受信
     * バッファへセットする（SF: 06 19 04 00 01 01 01）。 */
    ...
    FakeCanHw_RxData[2] = DCM_DTC_SUBFUNC_REPORT_SNAPSHOT;
    FakeCanHw_RxData[3] = 0x00U;  // DTC_H
    FakeCanHw_RxData[4] = 0x01U;  // DTC_M
    FakeCanHw_RxData[5] = 0x01U;  // DTC_L
    FakeCanHw_RxData[6] = DCM_FREEZEFRAME_RECORD_NUMBER;
    FakeCanHw_RxPendingCount = 1U;

    /* 実行 (Act 1): リクエスト受信 → Dcm 応答生成 → CanTp_Transmit(18バイト)
    ...
    EXPECT_EQ(FakeCanHw_LastSendData[0], 0x10U);  // FF PCI（len=18<256）
    EXPECT_EQ(FakeCanHw_LastSendData[1], 18U);    // len 下位8bit
    ...
    /* 実行 (Act 3): 残り12バイトは Consecutive Frame 2本（7+5バイト）で
     * 運びきれる（ceil((18-6)/7)=2）。 */
     ...
     * 内容を結合すると Dcm が生成した元の18バイト UDS ペイロードと完全
     * 一致すること。 */
    ASSERT_EQ(pos, 18U);
...
{
    /* 準備 (Act 1): DEM_EVENT_ENGINE_OVERHEAT を FAILED 確定させ FreezeFrame
     * を記録させてから、recordNumber=0xFF ([0x19, 0x04, 0x00,0x01,0x01, 0xFF])
     * を 0x7E0 の受信バッファへセットする（SF: 06 19 04 00 01 01 FF）。 */
    (void)Dem_SetEventStatus(DEM_EVENT_ENGINE_OVERHEAT, DEM_EVENT_STATUS_FAILED);
    (void)Dem_SetEventStatus(DEM_EVENT_ENGINE_OVERHEAT, DEM_EVENT_STATUS_FAILED);
     ...
     * 内容を結合すると Dcm が生成した元の18バイト UDS ペイロード（実レコード
     * 番号0x01を含む）と完全一致すること。 */
    ASSERT_EQ(pos, 18U);
    EXPECT_EQ(FakeCanHw_SendCount, 3U);  // FF 1 + CF 2
    EXPECT_EQ(CanTp_IsTxBusy(), (boolean)0U);
    ...
    EXPECT_EQ(reassembled[6], DCM_FREEZEFRAME_RECORD_NUMBER);  // 要求0xFFをechoしない
}

// ------------------------------------------------------------
// NG: DTC/recordNumber が揃わない（[0x19, 0x04, DTC_H, DTC_M] のみ、4バイト。
...
{
    /* 準備 (Arrange): DTC/recordNumber が揃わない [0x19, 0x04, 0x00, 0x01]
     * を 0x7E0 の受信バッファへセットする（SF: 04 19 04 00 01）。 */
    FakeCanHw_RxId  = 0x7E0U;
    FakeCanHw_RxDlc = 8U;
...
// (NRC 0x31) になる（Dem_SetEventStatus() を一切呼ばない、FreezeFrame
// 未記録の状態のまま）。
// ------------------------------------------------------------
TEST_F(Bsw_DcmStack_SID19_SF04_ReadDtcSnapshotChain_Test,
       ReadDtcSnapshot_NG_NeverFailedDtcProducesRequestOutOfRangeResponseOnCanHw)
{
    /* 準備 (Arrange): FreezeFrame 未記録のまま
     * [0x19, 0x04, 0x00,0x01,0x01, recordNumber=0x01] を 0x7E0 の受信
     * バッファへセットする（SF: 06 19 04 00 01 01 01）。 */
    FakeCanHw_RxId  = 0x7E0U;
    FakeCanHw_RxDlc = 8U;
