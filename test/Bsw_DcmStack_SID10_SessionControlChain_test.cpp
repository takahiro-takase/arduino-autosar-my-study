/**
 * \file    Bsw_DcmStack_SID10_SessionControlChain_test.cpp
 * \brief   UDS SID 0x10 DiagnosticSessionControl の、物理層（Can_Hw フェイク）
 ...
 *          `SessionControl_NG_ExtraByteReturnsIncorrectMessageLength`
 *          （固定長サービスの上限長チェック、2026-09 是正の回帰確認）を
 *          ここへ移植する。他の DcmStack チェーンテストは全て
 *          `EnterExtendedSession()` の前提確立ヘルパーとして SID 0x10 を
 *          間接的に使っているが、SID 0x10 自体を単独で対象とするファイルは
 *          本ファイルが初めてとなる。
 */
...
class Bsw_DcmStack_SID10_SessionControlChain_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_DcmStack_SID10_SessionControlChain_Test,
