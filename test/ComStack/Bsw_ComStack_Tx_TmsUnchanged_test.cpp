/**
 * \file    Bsw_ComStack_Tx_TmsUnchanged_test.cpp
 * \brief   README.md「Tx 処理（Com → PduR → CanIf → Can の順）」コールチェーンの
 *          単体テスト（GoogleTest / CMake native_chain_tests）。TmsUnchanged シナリオ専用。
 *
...
class Bsw_ComStack_Tx_TmsUnchanged_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ことを示す回帰テスト）。
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_TmsUnchanged_Test, TmsUnchanged_OK_DoesNotTriggerSendWithoutGroupTrigger)
{
