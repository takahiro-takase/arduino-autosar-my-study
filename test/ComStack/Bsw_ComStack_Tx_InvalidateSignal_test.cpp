/**
 * \file    Bsw_ComStack_Tx_InvalidateSignal_test.cpp
 * \brief   README.md「Tx 処理（Com → PduR → CanIf → Can の順）」コールチェーンの
 *          単体テスト（GoogleTest / CMake native_chain_tests）。InvalidateSignal シナリオ専用。
 *
...
class Bsw_ComStack_Tx_InvalidateSignal_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_InvalidateSignal_Test, InvalidateSignal_OK_WritesConfiguredInvalidValueToBuffer)
{
    /* 実行 (Act) */
...
TEST_F(Bsw_ComStack_Tx_InvalidateSignal_Test, InvalidateSignal_NG_UnconfiguredInvalidValueReturnsServiceNotAvailableWithoutWriting)
{
...
TEST_F(Bsw_ComStack_Tx_InvalidateSignal_Test, InvalidateSignal_NG_UnknownSignalIdReturnsError)
{
    /* 実行 (Act) + 評価 (Assert) */
    EXPECT_EQ(Com_InvalidateSignal(255U), E_NOT_OK);
}

TEST_F(Bsw_ComStack_Tx_InvalidateSignal_Test, InvalidateSignal_NG_RxSignalReturnsErrorWithoutReachingSendSignal)
{
