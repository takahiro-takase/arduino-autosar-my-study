/**
 * \file    Bsw_ComStack_Tx_IpduGroupStopTxTOutInteraction_test.cpp
 * \brief   README.md「Tx 処理（Com → PduR → CanIf → Can の順）」コールチェーンの
 *          単体テスト（GoogleTest / CMake native_chain_tests）。IpduGroupStopTxTOutInteraction シナリオ専用。
 *
 * \details 2026-09-21、Bsw_ComStack_Tx_IpduGroupStop_test.cpp から
...
class Bsw_ComStack_Tx_IpduGroupStopTxTOutInteraction_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
// Com_IpduGroupStop() と TX 送信デッドライン監視（Com_CbkTxTOut）の
// 二重発火防止。TxErrCbk とTxTOutCbk が同時に発火してしまわないことを
// 検証する。
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_IpduGroupStopTxTOutInteraction_Test, IpduGroupStopTxTOutInteraction_OK_PreventsDoubleFireWithTxErrCbk)
{
