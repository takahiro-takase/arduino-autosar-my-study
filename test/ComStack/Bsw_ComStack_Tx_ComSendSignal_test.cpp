/**
 * \file    Bsw_ComStack_Tx_ComSendSignal_test.cpp
 * \brief   README.md「Tx 処理（Com → PduR → CanIf → Can の順）」コールチェーンの
 *          単体テスト（GoogleTest / CMake native_chain_tests）。ComSendSignal シナリオ専用。
 *
...
class Bsw_ComStack_Tx_ComSendSignal_Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
// ------------------------------------------------------------
TEST_F(Bsw_ComStack_Tx_ComSendSignal_Test, ComSendSignal_OK_SetsPendingAndPacksBuffer)
{
    /* 準備 (Arrange) */
...
TEST_F(Bsw_ComStack_Tx_ComSendSignal_Test, ComSendSignal_NG_UnknownSignalId_DoesNotSetPending)
{
    /* 準備 (Arrange) */
    uint16_t value = 0x1234U;
