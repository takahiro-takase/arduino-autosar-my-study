...
- **TX（CAN ID 0x200）**: エンジン状態（OFF / STARTING / RUNNING / FAULT）を変化時送信＋周期フロア（ComFilterAlgorithm、AUTOSAR E2E Profile 01 保護付き）
...
|  | E2EXf | SWS_E2ELibrary 12.4 (E2E Transformer) | Com と E2E の間を仲介する統合層。RX は `E2EXf_InverseTransform` が `E2E_P01Check` を呼び、EngineInfo (CAN 0x100)・AbsInfo (CAN 0x110) のデータ破壊・フレーム脱落・重複・誤ルーティングを検出して Dem へそれぞれ DTC 0x00010A・0x000109 を報告。TX は `E2EXf_Transform` が `E2E_P01Protect` を呼び、MeterStatus (CAN 0x200) に Counter・CRC8 を付加。呼び出し元は Rte 層のグルー関数（`Rte_COMCbk_*`/`Rte_COMTransform_*`）で、Com 自身はこの層の存在を知らない |
|  | Com | SWS_Com | シグナルのビット単位パック／アンパックと受信デッドライン監視（タイムアウト検出）のみを担い、E2E には一切関知しない（E2E Transformer 方式）。I-PDU ごとの `RxIndicationCbk`/`TxTransformCbk`（`Com_IPduConfigType` の汎用フック、`Com_PBCfg.c` で設定）を通じて Rte 層のグルー関数を呼ぶだけで、中身が E2E であることも Com.c 本体には埋め込まれない。ComFilterAlgorithm により「送信すべきか」自体も Com が判断し、値が変化しない限り実送信を抑制する（周期フロアあり）。WarningStatus (0x210) は Signal Group として複数シグナルを `Com_SendSignalGroup()` でシャドウバッファから一括コミットする |
...
│   │   │   └── E2EXf_PBCfg.c     # I-PDU ごとの E2E P01 設定/状態実体（EngineInfo/AbsInfo/MeterStatus）
...
```
（修正前）
Com_TriggerIPDUSend() → PduR_Transmit() → CanIf_Transmit() → Can_Write()
...
その再送呼び出しがそのまま `Can_Write()` の再帰呼び出しになってしまう。CAN Bus-Off +
HW ウォッチドッグリセットの過去バグ（本 README 前半参照）と同じ「今は実害がないが
将来の変更で踏み抜きやすいスタック深化の地雷」のパターンであるため、NvM の非同期
書き込みジョブキューと同じ考え方で、この結合を断ち切った。
...
> **実機検証で得られた教訓（意図的な二重化）**: 実機検証の初回テストで、
> `Can_Isr()` が一度も起動されていないように見える現象が発生した
> （診断用に一時的に追加したカウンタが長時間 0 のまま）。後日の再テストでは
> 同じカウンタが正常にインクリメントしており、割り込み自体は機能することを
> 確認した。初回テストで 0 のままだった直接の原因は特定できていない
> （その時点でバス上に実際のフレームが流れていなかった可能性が高い）。
>
...
> しなくてもポーリング側だけで正しく動作することが実機で確認できている。
> 単一の検出経路（割り込みのみ）に正しさを委ねず、独立したポーリングでも
> 動作を保証する設計にしたこと自体が、実機検証を通じて得られた教訓である。
...
|-------|---------|--------|-----|-----------|--------|---------|----------|
| Tx | MeterStatus | 0x200 | 3 | ↓ | ↓ | ↓ | ↓ |
|  |  |  |  | 0–7 | 8 bit | E2E CRC | CRC8 SAE J1850<br>（DataID=0x0200 + byte[1-2] を対象に計算。AUTOSAR 標準バリアント 1A 準拠でCRCは先頭バイト） |
|  |  |  |  | 12–15 | 4 bit | E2E Counter | 0–15 のリングカウンタ（送信のたびに +1） |
|  |  |  |  | 16–23 | 8 bit | EngineState | 0=OFF<br>1=STARTING<br>2=RUNNING<br>3=FAULT |
...
**MeterStatus（メータ ECU / CAN ID 0x200 / DLC=3 / AUTOSAR E2E Profile 01 保護）**

`App_EngineManager_Run()`（3000ms 周期）が毎サイクル送信を要求しますが、実際に CAN へ
出るのは `EngineState` が変化したとき、または ComFilterAlgorithm の周期フロア（後述）に
達したときだけです（詳細は次項）。エンジン運転状態は他 ECU（盗難防止・ボディ制御等）の
判断材料になりうるため、一般的なメータ ECU のイグニッション/運転状態配信を模して AbsInfo
と同じ構成（データ＋Counter＋CRC）で E2E 保護しています。受信側の検証（Check）は行っておらず、
送信側の保護（`E2E_P01Protect`、`Com_TriggerIPDUSend()` が呼ぶ `TxTransformCbk` 経由で
Rte 層の `Rte_COMTransform_MeterStatus()` から呼び出される）のみを実装しています。
AbsInfo とは逆に、本 ECU はこのフレームの送信元であって受信者ではないためです。

**WarningStatus（メータ ECU / CAN ID 0x210 / DLC=1 / Com Signal Group）**

`App_WarningIndicator_Run()`（500ms 周期）が 3 本の LED レベルを計算した直後、同じ値を
Signal Group としてまとめて CAN へも送信します。E2E 保護は付与していません
（ダッシュボード表示用の LED ミラー情報であり、他 ECU の制御判断に使う想定がないため）。

#### ComFilterAlgorithm（送信要否を Com 自身が判断する）

実車の AUTOSAR Com は、ASW が値をセットする（`Com_SendSignal`）ことと、実際に CAN へ
送信すべきかを判断すること（`ComFilterAlgorithm` + 送信モード）を分離しています。
...
Com_SendSignal(ENGINE_STATE, value):
  ...
  (value & Mask) != (前回のフィルタ比較値 & Mask) ?
    YES → Com_TxUpdatePending[MeterStatus] = 1   ← 「送信すべき更新あり」
    NO  → 何もしない（バッファは更新済みだが送信要求は立たない）

Com_TriggerIPDUSend(MeterStatus)（ASW が 3000ms ごとに呼ぶ）:
  Com_TxUpdatePending[MeterStatus] == 1 ?
    YES → 送信する（TxTransformCbk で E2E Transform → PduR_Transmit）
    NO  → 呼び出し回数が COM_TX_PERIODIC_FLOOR_CYCLES (既定 3) 回に達した？
            YES → 周期フロアとして強制送信する
            NO  → 送信しない（"TX skip iPdu=0 (no change)" を DEBUG ログ）
```

**なぜ周期フロアが必要か**: 単純に「変化時のみ送信」にすると、`EngineState` が
...
一般的です。本プロジェクトでは `COM_TX_PERIODIC_FLOOR_CYCLES`（既定 3、
`Com_TriggerIPDUSend()` の呼び出し回数基準。ASW の呼び出し周期が 3000ms のため
実質 9000ms ごと）でこれを簡易的に再現しています。

**責務分離の効果**: 以前は「変化したら送る」という判断を ASW（`App_EngineManager`）が
毎サイクル無条件に `Com_TriggerIPDUSend()` を呼ぶことで暗黙的に実現していました
（実際には無条件に送信していただけで、判断はしていませんでした）。今は ASW は
一切変わらず「値をセットして送信を要求する」だけを行い、実際に送るかどうかは
Com 層が `ComFilterAlgorithm` の設定だけで決めます。将来 ASW のロジックを変更しても
Com の設定（`Com_PBCfg.c`）だけで送信頻度のポリシーを調整できます。

#### Com 設定（`Com_Cfg.h`）

| 定数 | 既定値 | 意味 |
|------|--------|------|
| `COM_TX_PERIODIC_FLOOR_CYCLES` | 3 | `Com_TriggerIPDUSend()` 呼び出し回数基準の周期フロア。変化がなくてもこの回数に達したら強制送信する |
...
```
Com_IPduConfigType.IsSignalGroup = 1（WarningStatus, IPduId=1）
...
Com_SendSignalGroup(GroupId=1)   ← 3 シグナルすべて設定し終えた後に 1 回呼ぶ
  ...
  前回コミット値（Com_GroupFilterLastBuffer[1]）とバイト比較
    変化あり → Com_TxUpdatePending[1] = 1   ← 「送信すべき更新あり」
    変化なし → 何もしない（Com_TriggerIPDUSend の周期フロアに委ねる）

Com_TriggerIPDUSend(1)（ASW が Signal Group コミット直後に呼ぶ）
  MeterStatus と同じロジックで送信要否を判定（Com_TxUpdatePending か周期フロアか）
```
...
`Com_TriggerIPDUSend()` を呼んでしまうと、3 本のうち更新済みのものと未更新のものが
混在した状態で送信されてしまいます。シャドウバッファへ一旦貯めてから
`Com_SendSignalGroup()` で一括コピーすることで、この不整合な中間状態が
実 TX バッファ（延いては CAN バス）へ現れることはありません。
...
## E2E P01 保護（EngineInfo/AbsInfo 受信 / MeterStatus 送信）
...
- **MeterStatus（CAN 0x200、送信）**: 本 ECU（メータ ECU）が送信するエンジン運転状態フレームに
  `E2E_P01Protect`で Counter・CRC8 を付加（本セクション後半）。他 ECU がエンジン運転状態を
  誤って読み取ることを防ぐため、一般的なメータ ECU のイグニッション/運転状態配信を模しています

> WarningStatus（CAN 0x210、送信）には E2E 保護を付与していません。こちらは
> EngineInfo/AbsInfo を Com が既に検証した**後**にメータ ECU 自身が導出する二次データ
> （警告灯の点灯状態）であり、実車でも一次センサ値ほど厳密な保護が付与されないことが
...
（`E2EXf_EngineInfoRxCfg` / `E2EXf_AbsInfoRxCfg` / `E2EXf_MeterStatusTxCfg` として
...
### 送信側（Protect）— MeterStatus

#### フレームレイアウト

```
byte[0]   : CRC8 SAE J1850（多項式 0x1D、初期値 0x00、最終 XOR 0x00、SWS_E2E_00083 準拠）
            計算対象: DataID_low(0x00), DataID_high(0x02), byte[1], byte[2]
            （CRC バイト自身を除く全バイト。CRC バイトより前の区間は 0 バイト）
byte[1]   : 上位 4bit=未使用、下位 4bit=Counter（0→1→…→14→0 のリングカウンタ、15 はスキップ）
byte[2]   : シグナルデータ（EngineState）
```
...
MeterStatus フレームは Counter=0 です。

#### Com モジュールとの統合（E2E Transformer 方式）

Com は MeterStatus のペイロードにも一切関知しません。`Com_TriggerIPDUSend()` は
PduR へ転送する**直前**に、`Com_IPduConfigType.TxTransformCbk`（`Com_PBCfg.c` で
`Rte_COMTransform_MeterStatus` を設定）を実 TX バッファへのポインタと長さ付きで
呼び出すだけです。実際に Counter・CRC8 を書き込むのは `Rte_COMTransform_MeterStatus()`
（`src/Rte/Rte.c`）で、中身は `E2EXf_Transform()`（`E2E_P01Protect()` への薄いラッパー）
を呼ぶだけです。AbsInfo の Check とは逆に、失敗や再送は発生しません
（送信側なので検証すべき前提がないため）。

```
Com_TriggerIPDUSend() (TxTransformCbk が設定された I-PDU。現状 IPduId=0 が対象):
  TxTransformCbk(Com_TxBuffer[PduId], DLC) を呼び出す
    = Rte_COMTransform_MeterStatus() （Rte.c）
...
#### E2E モジュール設定（`src/Bsw/E2EXf/E2EXf_PBCfg.c` の `E2EXf_MeterStatusTxCfg`）

| 設定 | 値 | 意味 |
|------|----|------|
| DataID | 0x0200 | CRC 計算に含む ID（AbsInfo と同様、CAN ID と一致） |
| DataLength | 3 | フレーム全体のバイト長 |
...
```
[6012ms] INFO  Can_Hw: TX OK id=0x200 dlc=3 [XX 00 02]
                                              └┘  └┘ └── EngineState=2（RUNNING）
                                              │   └───── Counter=0（初回送信）
                                              └───────── CRC8（自動計算）
[9012ms] INFO  Can_Hw: TX OK id=0x200 dlc=3 [YY 01 02]  ← Counter が 1 に進む
```
...
タイムアウトタイマとも更新しません）。**Tx 無効時の挙動**: `Com_TriggerIPDUSend()`
は実送信を抑制し、かつ ComFilterAlgorithm の評価結果（`Com_TxUpdatePending`）と
周期フロアのカウンタを破棄します（詳細は次項）。TX バッファの値自体は
`Com_SendSignal()` が既に更新済みのため失われず、再開後に実際に値が変化した時、
または通常の周期フロアに新たに達した時に初めて送信されます。

**Rx 無効中の受信デッドライン監視（コードレビューで発覚・修正済み）**:
当初、`Com_MainFunction()`（受信デッドライン監視、100ms 周期）は
`Com_RxEnabled` を一切参照していなかった。SWS_Com_00684/SWS_Com_00685
（`Com_IpduGroupStop` により I-PDU が止められた間は受信処理だけでなく
...
**Tx 無効中の送信トリガー保持（コードレビューで発覚・修正済み）**:
当初、`Com_TriggerIPDUSend()` は Tx 抑制中も `Com_TxUpdatePending`・
周期フロアのカウンタをあえて保持し、「再度有効化された瞬間に無効化中の
更新が送信される」設計にしていた（コメント上は意図的な設計としていたが、
仕様とは逆方向の判断だった）。しかし SWS_Com_00777「停止中の I-PDU の
送信要求はキャンセルしなければならない」、および SWS_Com_00334 の説明文
「停止中に発生した送信トリガーは保持されず、再開しても古いトリガーで
即座に送信されることはない」に反していた。`Com_TriggerIPDUSend()` は
Tx 抑制中を検出した時点で `Com_TxUpdatePending`/`Com_TxCyclesSinceSent` を
破棄するよう修正した。TX バッファの値自体は `Com_SendSignal()` が既に
更新済みのため失われず、再開後に実際に値が変化した時、または通常の
周期フロアに新たに達した時に初めて送信される（＝仕様どおり「即座には
送信されない」）。
...
**なぜ非同期化したか**: 当初 `NvM_WriteBlock()` は EEPROM への書き込みも含めて
完全に同期処理でした。Renesas RA の EEPROM ライブラリ（内蔵フラッシュの
エミュレーション）は 1 バイトの書き込みでも消去・書き込みサイクルを伴うため、
9 バイト超のブロック（DEM_STATUS 等）をまとめて同期的に書くと数百 ms
協調スケジューラ全体が停止することが実機で判明しました。この停止は
`Dem_ReportErrorStatus()` が新規 DTC 確定のたびに発生し、WdgM の
Deadline Supervision（他タスクの実行間隔を監視する仕組み）を巻き込んで
実際に HW ウォッチドッグリセットを引き起こしていました
（詳細は WdgM セクションの「複数 Supervised Entity」の実機発見ログを参照）。
閾値を広げる対症療法ではなく、ブロッキングそのものを解消するため、
実際の AUTOSAR NvM と同じ非同期ジョブキュー方式へ変更しました。
...
> ことはありません（`Can_T_STOP`/`Can_T_START` の間を往復するのみ）。以前は
> 回復を断念して専用の恒久スリープ（AUTOSAR 標準外の `Can_EnterFinalSleep()`）へ
> 落とす経路がありましたが、AUTOSAR 仕様（SWS_CanSM_00514/00515/00636）に
> 「回復を諦めて二度と復帰しない」状態が存在しないことが判明したため撤去し、
> 無期限リトライへ置き換えました。
...
#### 実機で見つかった不具合: ウェイクアップ直後の再集約による即座の再スリープ

ウェイクアップ検証の実機テスト中に、次のような不具合が見つかりました。
...
`extendedSession` への切替では再現せず、`defaultSession` への切替でのみ再現していました。
原因は `ComM_BusSMIndication()` にありました。CanSM がウェイクアップ検証成功時に
`ComM_BusSMIndication(FULL_COM)` を呼んでチャネル状態を更新しても、**どのユーザの
要求でもない自動的な変化**であるため `ComM_UserRequest[COMM_USER_0]` は更新されず、
スリープ突入時の古い値（`NO_COM`）のまま残っていました。

`App_EngineManager_Run()` がまだ 1 周期も再評価していない（Task 2 が次に実行される
のは最大3000ms後）そのわずかな間に、受信したフレームの内容が defaultSession への
SessionControl だったため、Dcm が `ComM_RequestComMode(COMM_USER_1, NO_COM)` を
呼びました。この時点で再集約すると `max(User0=NO_COM（古い値）, User1=NO_COM) = NO_COM`
となり、実際にはウェイクアップ検証を通過してチャネルが FULL_COM になったばかりにも
関わらず、即座に再スリープしてしまっていました。`extendedSession` の場合は
`User1=FULL_COM` になるため、User0 の古い値が埋もれて表面化しなかっただけです。

**修正**: `ComM_BusSMIndication()` が `FULL_COM`/`NO_COM` を通知するとき、
`ComM_UserRequest[COMM_USER_0]` もその値へ同期するようにしました。CanSM 側の
自動的な状態変化を「User0 の暫定的な要求」とみなすことで、App_EngineManager が
次に実際のエンジン状態に基づいて要求し直すまでの間、矛盾のない値を保持できます。
...
> **実機で発見した意外な落とし穴（Deadline (END→START) の上限緩和）**:
> 当初 WARNING の END→START 上限は 700ms（500ms 周期に ±200ms/±40%）でしたが、
> 障害注入を伴わない通常動作でも Deadline Supervision が FAILED になり、
> 実際に HW ウォッチドッグリセットが発生しました（`elapsed=741ms`）。
> 原因は `Dem_ReportErrorStatus()` が新規 DTC 確定時に `NvM_WriteBlock()` 経由で
> EEPROM へ同期書き込みすること。Renesas RA の `NvM_Hw_WriteBlock()`
> （`src/Hal/NvM_Hw.cpp`）はバイト単位の `EEPROM.update()` ループで、
> フラッシュの消去・書き込みを伴うブロッキング処理のため、DTC 確定のたびに
> 協調スケジューラが数百ms 単位で止まります。この遅延は WARNING タスク自身の
> 異常ではなく他 BSW モジュール（Dem/NvM）由来であるため、ENGINE と WARNING
> 双方の END→START 上限に実測値の約2倍の余裕を持たせました（ENGINE: 3500→4500ms、
> WARNING: 700→1500ms）。500ms 周期の WARNING は 3000ms 周期の ENGINE より
> チェックポイント報告の間隔が短いため、この種の「他モジュール起因の一時的な
> ブロッキング」の影響を相対的に受けやすいという教訓が得られました。
>
> **さらに見つかった本質的な原因（`Os_SchedulerStep()` のバグ）**:
> 上記の上限緩和後も、今度は `elapsed=224ms`（下限 300ms を下回る、短すぎる間隔）
> という逆方向の Deadline 違反が発生しました。原因は `Os_SchedulerStep()`
> （`src/Os/Os.c`）が `now = millis()` をループの**先頭で 1 回だけ**取得し、
> 全タスクの周期判定・`Os_LastRunMs[]` 更新に使い回していたことです。
...
> 記録されてしまいます。次回のスキャンでは、既に進んだ実時刻との差分が
> 本来より大きく計算され、周期到来判定が早まって短い間隔で再実行されて
> しまいます。Deadline の閾値をいくら調整してもこの問題は解消できず、
> `now` をタスクごとにループ内で毎回取得し直すよう `Os_SchedulerStep()`
> 自体を修正しました。
>
> **もう一つの落とし穴（SHUTDOWN から RUN 復帰直後の Alive Supervision 誤検出）**:
> 上記の修正後、今度はボランタリスリープ → SHUTDOWN → CAN ウェイクアップによる
> RUN 復帰の直後に `WARN WdgM: SE1 alive FAILED alive=2 (exp>=6)` が発生し、
> 実際にリセットしました。原因は `WdgM_MainFunction`（Task 7、6000ms 周期）
> 自身が SHUTDOWN 中は `BSWM_TASK_MASK_SHUTDOWN` により無効化されており、
> その `Os_LastRunMs[]` が無効化前の古い時刻のまま残っていたことです。
> SHUTDOWN が 6000ms 以上続いた後に RUN へ復帰すると、`Os_SchedulerStep()` は
> 「経過時間が周期を大幅に超えている」と判定し、`WdgM_MainFunction` を
> 再開直後にほぼ即座に実行してしまいます。この時点では WARNING タスクが
> まだ 1〜2 回しか実行されておらず（`AliveCount` が閾値 6 に届かない）、
> 本来なら復帰後まるまる 6000ms 監視して初めて判定すべきところを、
> 実行機会がほとんどないまま Alive Supervision が FAILED と誤判定していました。
> `WdgM_ResumeSupervision()`（チェックポイント基準のリセット）と同種の問題が
> OS タスクスケジューリングのレベルでも起きていたことになります。
> 修正として、`Os_SetTaskActive()` がタスクを無効→有効へ切り替える瞬間に
> `Os_LastRunMs[]` を現在時刻へリセットするようにしました。これにより、
> 長時間無効化されていたどのタスクも、再開直後は必ずフルの周期を待ってから
> 初めて実行・評価されるようになります。
...
#### グローバルレベルの EXPIRED 許容サイクル（コードレビューで発覚・修正済み）

当初、`WdgM_TriggerHwWatchdog()` は「1 つでもエンティティが FAILED ならその場で
リフレッシュを止める」設計でした。しかし `docs/AUTOSAR_SWS_WatchdogManager.pdf`
を確認すると、`[SWS_WdgM_00119]`〜`[SWS_WdgM_00121]` は Global Supervision
Status が `WDGM_GLOBAL_STATUS_OK`・`FAILED`・`EXPIRED` のいずれであっても
`WdgIf_SetTriggerCondition`（リフレッシュ相当）を同一に呼び続けることを要求
しており、リフレッシュを 0（停止）にしてよいのは `[SWS_WdgM_00122]`
`WDGM_GLOBAL_STATUS_STOPPED` に到達したときだけでした。STOPPED に到達するには
`WdgMExpiredSupervisionCycleTol`（グローバルレベルの EXPIRED 許容サイクル数）
分の判定サイクルを消費する必要があり（`[SWS_WdgM_00216]`/`[SWS_WdgM_00217]`
等）、仕様は単発の異常でいきなりリフレッシュを止めることを想定していません。

実際、このファイルの `WDGM_ENGINE_DEADLINE_END_TO_START_MAX_MS` /
`WDGM_WARNING_DEADLINE_END_TO_START_MAX_MS`（`WdgM_Cfg.h`）には、「NvM の
EEPROM ブロッキング書き込みで数百ms（最大 elapsed=741ms）のスケジューラ停止が
起き、Deadline 違反を誤検出した」という実機不具合の経緯が記録されています。
これはまさに、仕様の猶予機構が本来吸収すべき「一時的なスケジューラ遅延」を、
タイミングマージンの拡張という対症療法だけで凌いでいた状態でした。

修正として、`WdgM_ExpiredCycleCount`（グローバルレベルの連続 FAILED 判定
サイクル数）と `WdgM_GlobalStopped`（AUTOSAR の `WDGM_GLOBAL_STATUS_STOPPED`
相当）を追加しました。本実装は Local Supervision Status を OK/FAILED の
...
18000ms の連続した猶予消費が必要だったことです。RUN 復帰のたびに
...
回復しないラッチ式の設計です。それを評価するグローバル猶予カウンタだけを
resume のたびに回復させてしまうのは非対称で誤りでした。修正として
`WdgM_ResumeSupervision()` からこの 2 つのリセットを削除し、真に全
エンティティが OK に戻ったとき（`WdgM_MainFunction()` 末尾の自然な回復
判定）にのみクリアされるようにしました。これにより、恒久的な違反は
何回スリープ/ウェイクアップを挟んでも判定サイクル換算で着実に猶予を
消費し続け、いずれ確実に `WdgM_GlobalStopped` に到達するようになります。

あわせて、`WdgM_SupervisionSuppressed`（POST_RUN 中の想定内の Alive
不足を無視するフラグ）が立っている間は、グローバル猶予カウンタの判定
自体を凍結する（進めも回復させもしない）ようにしました。POST_RUN 中の
Rte_Engine/Rte_Warning 停止による想定内の Alive 不足が、POST_RUN の
頻度や長さ次第でグローバル猶予を無関係に消費してしまうことを防ぐためです。
...
(`WdgM_LastCheckpointTimeMs[]`) は停止前の古い値のまま残ります。これをリセット
せずに RUN へ復帰すると、再開後最初のチェックポイントで、END→START の経過時間
として **POST_RUN の停止時間（最大 `ECUM_POST_RUN_TIMEOUT_MS`=5000ms）** を
計算してしまい、ENGINE・WARNING いずれの許容上限（それぞれ 3500ms / 700ms）も
確実に超過し、誤って Deadline Supervision が FAILED と判定 → ラッチされてしまいます。
...
> 現在は AVR・Renesas RA いずれも HW ウォッチドッグが実際に有効化されているため、
> この不具合を修正せずに POST_RUN→RUN を経由すると、この誤判定がそのまま
> 予期しない MCU リセットを引き起こす（`WdgM_ResumeSupervision()` は既に
> 修正として組み込み済みで、現状は再現しない）。

#### POST_RUN→RUN 復帰時の Alive Supervision 誤検出（CommunicationControl 実機検証中に発覚・修正済み）

UDS CommunicationControl（SID 0x28）の実機検証中、S3 タイムアウト →
ボランタリスリープ → POST_RUN → SHUTDOWN → CAN ウェイクアップによる RUN 復帰、
という一連の流れの直後に実際に HW ウォッチドッグリセットが発生しました。

POST_RUN 中は Rte_Engine / Rte_Warning タスクが意図的に停止するため、
`WdgM_MainFunction()` の判定サイクルで両エンティティの Alive Supervision が
必ず FAILED とラッチされます。POST_RUN 中はこれ自体は無害です
（`WdgM_SupervisionSuppressed` により `WdgM_TriggerHwWatchdog()` がリフレッシュ
拒否を無視するため）。しかし `WdgM_ResumeSupervision()` はチェックポイント基準
（`WdgM_LastCheckpoint[]` / `WdgM_LastCheckpointTimeMs[]`、上記 Deadline 誤検出
対策で導入）のみリセットし、`WdgM_AliveCount[]` / `WdgM_AliveStatus[]` は
リセットしていませんでした。そのため POST_RUN 中に付いた FAILED ラッチが
SHUTDOWN を越えて RUN 復帰後まで残ってしまいます。
...
続けます。次に `WdgM_MainFunction()` が判定し直してラッチが自然に解消される
のは最大 6000ms 後ですが、HW ウォッチドッグのタイムアウトは 4000ms しかない
ため、判定が間に合わず実際に MCU がリセットされてしまいました。

修正として、`WdgM_ResumeSupervision()` でチェックポイント基準に加えて
`WdgM_AliveCount[]`（0 に）・`WdgM_AliveStatus[]`（OK に）も全エンティティ分
リセットするようにしました。Logical/Deadline のラッチはリセットしません
（POST_RUN 中に新規発生することがなく、Alive とは異なり純粋な RUN 中の異常
のみを検出する項目のため、消してしまうと本当の違反を見逃すことになります）。

```
WdgM_ResumeSupervision():
  WdgM_LastCheckpoint[]       ← WDGM_CP_INITIAL にリセット（既存）
  WdgM_LastCheckpointTimeMs[] ← 現在時刻にリセット（既存）
  WdgM_AliveCount[]           ← 0 にリセット（NEW）
  WdgM_AliveStatus[]          ← WDGM_LOCAL_STATUS_OK にリセット（NEW）
```

実機で POST_RUN→SHUTDOWN→wakeup サイクルを再現し、修正後は
`HW watchdog NOT refreshed` が発生せず正常稼働が継続することを確認済みです。

#### 短時間 POST_RUN での Alive Supervision 誤検出（E2E Counter 修正の実機検証中に発覚・修正済み）

上記の修正後も、ボランタリスリープに入った直後（POST_RUN 遷移から 1 秒未満）に
バス活動を検知して即座にウェイクアップし RUN へ復帰する、という短い
POST_RUN のシナリオで、再度実際に HW ウォッチドッグリセットが発生しました。

原因は `WdgM_MainFunction()`（Task 7、Alive/Logical/Deadline を判定するタスク
自身）が POST_RUN 中も継続動作するタスクであるため、その呼び出しタイミング
（Os の内部スケジュール `Os_LastRunMs[7]`）が `WdgM_ResumeSupervision()` の
`WdgM_AliveCount[]` リセットと同期していないことです。POST_RUN が
`WDGM_SUPERVISION_CYCLE_MS`（6000ms）よりも大幅に短いと、リセット直後に
たまたま `WdgM_MainFunction()` の次回呼び出しタイミングが来てしまうことが
あります。この場合、Rte_Engine（3000ms 周期）等のエンティティが
`WdgM_ResumeSupervision()` 以降まだ一度もチェックポイントへ到達できていない
うちに判定が行われ、`alive=0 (exp>=1)` として誤って FAILED と判定 →
`WdgM_TriggerHwWatchdog()` がリフレッシュを拒否し続け、実際に MCU が
リセットされてしまいました。

上記の修正（`WdgM_AliveCount[]`/`WdgM_AliveStatus[]` のリセット）だけでは、
「リセットするタイミング」と「判定するタイミング」がそもそも同期していない
という、より根本的な問題は解消できていなかったことになります。

修正として、`WdgM_ResumeSupervision()` が `WdgM_SkipNextAliveJudgment` フラグを
立て、`WdgM_MainFunction()` は次回呼び出し 1 回分だけ Alive Supervision の
判定そのものをスキップするようにしました。`WdgM_AliveCount[]` には触れない
（リセットしない）ため、スキップした分の蓄積は次回の判定へそのまま持ち越され
ます。Os 自身のスケジューリングにより次回呼び出しは必ず一定時間後になるため、
その頃には各エンティティが十分チェックインを済ませており、正しく判定できます。

```
WdgM_ResumeSupervision():
  ...（上記の AliveCount/AliveStatus リセットに加えて）
  WdgM_SkipNextAliveJudgment ← 1（NEW）

WdgM_MainFunction():
  WdgM_SkipNextAliveJudgment が立っている ?
    YES → フラグを下ろして即 return（AliveCount は触らない、判定もしない）
    NO  → 通常どおり判定
```
...
サイクルで Alive 条件を満たすだけで検出した違反が消えてしまう問題がありました
（本プロジェクトでも実際に確認・修正済み）。Deadline Supervision の追加にあたり、
この独立ステータス配列のパターンをそのまま 3 つ目にも適用しています。
...
| `WDGM_DEADLINE_START_TO_END_MIN_MS` / `_MAX_MS` | 0 / 500 ms | START→END（Run() 1 回分）の許容経過時間 |
| `WDGM_DEADLINE_END_TO_START_MIN_MS` / `_MAX_MS` | 2500 / 3500 ms | END→START（次サイクルまでの間隔）の許容経過時間 |
...
`MeterStatus`（E2E 保護対象）は ASW → RTE → Com → PduR → CanIf → Can という
通常のシグナル送信経路を通ります。一方 `Nm` はシグナル値を運ばず、実車の `CanNm` も
Com スタックを経由せず直接 `CanIf_Transmit()` を呼ぶため、本プロジェクトの `Nm.c` も
同じ構造にしています。

```
MeterStatus: App_EngineManager → Com_SendSignal → Com_TriggerIPDUSend
...
[6011ms] INFO  Can: TX id=0x200 [XX 00 02]        # byte[0]=CRC8(E2E_P01Protect が付加), byte[1]=Counter(0), byte[2]=EngineState(2=RUNNING)
[6012ms] INFO  Can_Hw: TX OK id=0x200 dlc=3 [XX 00 02]
