...
|  | WdgM | SWS_WdgM | Supervised Entity の Alive Supervision（呼び出し回数を6000msごとに評価）・Logical Supervision（チェックポイント順序）・Deadline Supervision（チェックポイント間の経過時間）を独立したステータスで管理。異常時は AVR 実 HW ウォッチドッグのリフレッシュを止め、実際に MCU をリセットする |
...
|  | Dem | SWS_Dem | 診断イベントを DTC として管理。カウンタベースのデバウンスで確定し、NvM 経由で EEPROM に永続化。デバウンス確定 FAILED 時に FreezeFrame（RAM のみ）を記録し、ExtendedData（故障確定回数、NvM 永続化）を+1。再故障せず複数回の操作サイクルを経ると経年回復（Aging）で CONFIRMED を自動解除 |
...
WAIT_CF ─ CF 受信(未完) ────────────→ WAIT_CF
        ...
        ─ N_Cr タイムアウト(150ms) ─→ IDLE (中断)
        ─ 別の FF 受信 ─────────────→ WAIT_CF (無視。後述)
```

#### WAIT_CF 中に別の FF を受信した場合（busy 判定）

複数フレームの再組立中（`WAIT_CF`）に、別の新しい FF を受信した場合は無視します。
送信側 `CanTp_Transmit()` が送信中（`CANTP_TX_WAIT_FC` / `CANTP_TX_SEND_CF`）に
新規送信要求を `E_NOT_OK` で拒否する busy 判定と対称的な振る舞いです。

無視する理由は、進行中の再組立バッファを不用意に破棄しないためです。新しい FF を
無条件に受理してしまうと、それまで受信していた CF の断片が黙って失われ、
元のメッセージの組み立てが永久に失敗します。代わりに新しい FF を無視すれば、
進行中のセッションは N_Cr タイムアウト（150ms）で自然に中断されるため、
新しい FF はその後の再送で正常に受理されます（待ち時間は最大 150ms 程度）。

```
[1000ms] INFO  CanTp: RX FF len=20          ← 1 つ目の FF (WAIT_CF へ)
[1010ms] WARN  CanTp: RX FF ignored: already WAIT_CF (busy)  ← 2 つ目の FF は無視
[1150ms] ERROR CanTp: RX N_Cr timeout abort  ← 1 つ目が N_Cr タイムアウトで中断
[1200ms] INFO  CanTp: RX FF len=20          ← 再送された FF は正常に受理される
```
...
NvM_WriteBlock() / NvM_ApplyDefault() の書き込み時:
  データ本体を EEPROM へ書き込み
  CRC を再計算し、データ本体直後の 1 バイトへ書き込み
```
...
| ENGINE_SPEED_NO_FLAG / COMM_TIMEOUT / BUTTON_STUCK / ADC_VOLT_LOW | 3 | 標準 |
...
Arduino UNO の内蔵 EEPROM 先頭 29 バイトを使用します（NvM の CRC バイトを含む。
...
| 0x02 | NVM_BLOCK_ID_DEM_STATUS (8 bytes) | EVENT_ENGINE_OVERHEAT ステータス |
...
| 0x0A | 〃 CRC (1 byte) | STATUS ブロックの CRC8 |
| 0x0B | NVM_BLOCK_ID_DEM_AGING (8 bytes) | EVENT_ENGINE_OVERHEAT 経年回復カウンタ |
| 0x0C | 〃 | EVENT_ENGINE_STALL 経年回復カウンタ |
| 0x0D | 〃 | EVENT_ENGINE_SPEED_NO_FLAG 経年回復カウンタ |
| 0x0E | 〃 | EVENT_STARTING_TIMEOUT 経年回復カウンタ |
| 0x0F | 〃 | EVENT_COMM_TIMEOUT 経年回復カウンタ |
| 0x10 | 〃 | EVENT_BUTTON_STUCK 経年回復カウンタ |
| 0x11 | 〃 | EVENT_ADC_VOLT_LOW 経年回復カウンタ |
| 0x12 | 〃 | EVENT_CAN_BUSOFF 経年回復カウンタ |
| 0x13 | 〃 CRC (1 byte) | AGING ブロックの CRC8 |
| 0x14 | NVM_BLOCK_ID_DEM_EXTENDED (8 bytes) | EVENT_ENGINE_OVERHEAT 故障確定回数 |
| 0x15 | 〃 | EVENT_ENGINE_STALL 故障確定回数 |
| 0x16 | 〃 | EVENT_ENGINE_SPEED_NO_FLAG 故障確定回数 |
| 0x17 | 〃 | EVENT_STARTING_TIMEOUT 故障確定回数 |
| 0x18 | 〃 | EVENT_COMM_TIMEOUT 故障確定回数 |
| 0x19 | 〃 | EVENT_BUTTON_STUCK 故障確定回数 |
| 0x1A | 〃 | EVENT_ADC_VOLT_LOW 故障確定回数 |
| 0x1B | 〃 | EVENT_CAN_BUSOFF 故障確定回数 |
| 0x1C | 〃 CRC (1 byte) | EXTENDED ブロックの CRC8 |
...
[6017ms]  DEBUG WdgM: SE0 alive OK alive=4   ← 3000ms 周期で Run() が 2 回、各 2 チェックポイント
[6018ms]  DEBUG WdgM: HW watchdog refreshed
[12017ms] DEBUG WdgM: SE0 alive OK alive=4
...
（もし次の MainFunction サイクルが先にリセットされず実行された場合は
 [6017ms] WARN WdgM: SE0 logical still FAILED (latched since violation) [HW WDT reset pending]
 のように、Alive 条件を満たしていても Logical の FAILED は消えない。
 通常は以下の Alive 失敗検知と同様、約 2000〜8000ms 後に MCU が実際にリセットされる）
```
...
（以降は Logical 失敗検知と同様、latched ログを経て約 2000〜8000ms 後に
 MCU が実際にリセットされる）
```
