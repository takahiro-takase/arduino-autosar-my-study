...
│       │   └── IoHwAb.c          # Dio へ委譲・デバウンス（40ms）・固着検出（5 秒超過で Dem 報告）
...
## IO スタック（IoHwAb / Dio / Port）

SW-C はピン番号を直接知りません。RTE の Client/Server ポートを通じて IoHwAb の論理 API を呼び出し、
IoHwAb が Dio チャネルへ変換します。ピン方向の初期設定は Port が担い、Dio は値の読み書きのみを行います。

```
SW-C (App_EngineManager / App_WarningIndicator)
  │ Rte_Call_LedRunning_SetLevel / Rte_Call_Button_GetLevel 等
  ↓
IoHwAb（論理 API：LED / ボタン）
  │ Dio_WriteChannel / Dio_ReadChannel
  ↓
Dio（値の読み書き）
  │ Dio_Hw_WriteChannel / Dio_Hw_ReadChannel
  ↓
Dio_Hw（Arduino digitalWrite / digitalRead）
...
| `IoHwAb_MainFunction()` | Os Task 6 (10ms) | デバウンスサンプリング・固着検出 |
