...
|  | Dcm | SWS_Dcm | UDS 診断サービス処理（SID 0x10 / 0x11 / 0x14 / 0x19 / 0x22 / 0x27 / 0x3E）。S3 タイマでセッションを自動失効。SecurityAccess (0x27) で ClearDTC をシード・キー認証保護 |
...
| 0x14<br>ClearDiagnosticInformation | — | `04 14 FF FF FF 00 00 00` | byte2-4=groupOfDTC<br>・0xFFFFFF=全クリア<br>・DTCコード指定=1件クリア | 0x54<br>**SecurityAccess Level1 必須**（未認証は NRC 0x33） |
...
| 0x27<br>SecurityAccess | 0x01<br>(requestSeed) | `02 27 01 00 00 00 00 00` | — | 0x67（seed 2 バイト） |
|  | 0x02<br>(sendKey) | `04 27 02 HH LL 00 00 00` | byte2-3=key（big-endian） | 0x67 |
...
# UDS SID 0x14: 未アンロックで ClearAllDTC を試みる → NRC 0x33 で拒否
