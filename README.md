...
`variables { }` での変数宣言 (`byte`/`int`/`float` 配列、`message` 変数を含む)・
...
ビット演算子 (`>>`/`&`) による DID の上位/下位バイト分解を使った例）を
参照してください。
