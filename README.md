...
抽象化されたデータは RTE を介して 2 つの ASW SW-Component に提供され、
SWC 間でエンジン状態を共有しながら、警告灯（LED）の制御を行います。
...
[11ms] INFO  WarnInd: Init
[12ms] INFO  Os: Init ok tasks=4          # タスクスケジューラ起動（全モジュール初期化後）
