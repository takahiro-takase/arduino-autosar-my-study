...
│       ├── Dem/                  # 診断イベントマネージャ（DTC 管理・EEPROM 永続化）
...
Dem (Diagnostic Event Manager) モジュールがエンジン管理の故障を DTC として管理し、
Arduino UNO の内蔵 EEPROM（1 KB）へ永続化します。
...
| アドレス | 内容 |
|---------|------|
| 0x00 | マジックバイト（0xDE = 有効データあり） |
| 0x01 | EVENT_ENGINE_OVERHEAT ステータス |
...
[CanTp_Init] initialized
[Dcm_Init] session=Default
[Dem_Init] restored from NvM       # 2 回目以降の起動
  events=4
...
# UDS 診断要求 0x7E0（CanTp SF 受信）
[Mcp2515_Read] OK >>>> ID=0x7E0 DLC=8 DATA=[03 22 01 01 00 00 00 00]
[CanIf_RxInd] CanId=0x7E0 -> RxPduId=1
[CanTp_Rx] SF udsLen=3
[Dcm] SID=0x22
[Dcm 0x22] DID=0x101 len=2
[CanTp_Tx] SF len=5
[Mcp2515_Send] OK >>>> ID=0x7E8 DLC=8 DATA=[05 62 01 01 01 F4 00 00]

# 過熱検出 → DEM へ FAILED 報告
[EngineManager] RUNNING->FAULT(overheat=101)
[Dem] FAILED ev=0 DTC=0x000101     # EEPROM へ書き込み

# UDS SID 0x19/02: DTC 一覧取得（2 件以上 → マルチフレーム応答）
[CanTp_Rx] SF udsLen=4
[Dcm] SID=0x19
[Dcm 0x19/02] mask=0xFF found=2
[CanTp_Tx] FF len=11               # 11 バイト > 7 バイト → FF+CF に分割
[Mcp2515_Send] OK >>>> ID=0x7E8 DLC=8 DATA=[10 0B 59 02 2D 00 01 03]
# (Cangaroo から FC を受信後)
[CanTp_Rx] FC fs=0                 # ContinueToSend
[CanTp_Tx] CF sn=1 pos=6
[Mcp2515_Send] OK >>>> ID=0x7E8 DLC=8 DATA=[21 2C 00 01 04 2C 00 00]
[CanTp_Tx] complete

# UDS SID 0x14: 全 DTC クリア
[CanTp_Rx] SF udsLen=4
[Dcm] SID=0x14
[Dcm 0x14] ClearAllDTCs
[Dem] All DTCs cleared
[CanTp_Tx] SF len=1
[Mcp2515_Send] OK >>>> ID=0x7E8 DLC=8 DATA=[01 54 00 00 00 00 00 00]
```
