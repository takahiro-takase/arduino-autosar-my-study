...
システムは、エンジン ECU から CAN バス経由で送信されるエンジン状態を受信し、
BSW（CanDrv / CanIf / PduR / Com）が信号を抽象化します。
...
- **RX（CAN ID 0x100）**: 外部ツール（エンジン ECU 相当）から速度・水温・ON フラグを受信
...
- **警告灯 LED（D8）**: EngineState に応じて点灯 / 点滅 / 消灯を制御
...
│       App_WarningIndicator 警告灯 LED 制御            │
├──────────────────────────────────────────────────────┤
...
│       Dio    デジタル I/O 抽象化（MCAL）              │
├──────────────────────────────────────────────────────┤
│  HAL  Can_Hw  MCP2515 C++ ラッパー                    │  C++ のみ
│       Dio_Hw  Arduino GPIO ラッパー                   │  C++ のみ
└──────────────────────────────────────────────────────┘
```
...
│   │   └── App_WarningIndicator.c   # 警告灯 LED 制御（RUNNING=点灯, FAULT=点滅）
│   ├── Os/
│   │   ├── Os_Cfg.h              # タスク数定数
...
│       │   └── Dio_Hw.cpp        # Arduino pinMode / digitalWrite ラッパー（C++ のみ）
...
│       │   └── ComM.c            # 状態機械・Can_SetControllerMode へ委譲
...
[10ms] INFO  ComM: Init ch=1              # NO_COM 状態で初期化
[11ms] INFO  ComM: ch0 NO_COM/SILENT->FULL_COM  # 全層初期化後に CAN バス開通
[12ms] INFO  AppEng: Init->OFF
[13ms] INFO  IoHwAb: Init                 # LED チャネル (D8) を OUTPUT に設定・消灯
[14ms] INFO  WarnInd: Init
[15ms] INFO  Os: Init ok tasks=4          # タスクスケジューラ起動（全モジュール初期化後）
...
[9500ms] INFO  WarnInd: LED BLINK         # FAULT → 500ms ごとにトグル
[10000ms] INFO  WarnInd: LED BLINK
...
| `Dio_Hw.cpp` | C++ | Arduino の `pinMode` / `digitalWrite` API を使用 |
