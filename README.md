...
│       │   ├── Dio_Cfg.h         # チャネル ID 定義（LED=D8 / ボタン=D9）
...
│       │   ├── Port_Cfg.h        # ピン番号定義（LED=D8 OUTPUT / ボタン=D9 INPUT_PULLUP）
...
│       │   └── IoHwAb.c          # Dio へ委譲・INPUT_PULLUP 論理反転を吸収（Dio_Cfg.h を知る唯一の非 MCAL ファイル）
...
[2ms] INFO  Port: Init pins=2             # ピン方向設定（D8 を OUTPUT、D9 を INPUT_PULLUP に）
...
| 警告灯 LED / ボタンのピン番号変更 | `Dio_Cfg.h`（`DIO_CHANNEL_LED_WARNING` / `DIO_CHANNEL_BUTTON`） |
