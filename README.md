...
Alive Supervision と Logical Supervision は、それぞれ独立したステータス
（`WdgM_AliveStatus[]` / `WdgM_LogicalStatus[]`）で管理されます。
`WdgM_GetLocalStatus()` と HW ウォッチドッグの refresh 判定は、どちらか一方でも
FAILED なら FAILED として扱います。

- `WdgM_AliveStatus` は周期ごとに再評価され、Alive 条件を満たせば OK に戻ります。
- `WdgM_LogicalStatus` は `WdgM_Init()` までラッチされ、`WdgM_MainFunction` の
  周期処理では自動的に OK へ戻りません（不正な遷移が起きたという事実は、その後
  Alive 条件を満たしても消えないため）。

以前はこの 2 つを 1 つの `WdgM_LocalStatusType` に統合していたため、Logical
Supervision が FAILED と判定した直後でも、次の `WdgM_MainFunction` サイクルで
Alive 条件を満たすだけで検出した違反が消えてしまう問題がありました
（本プロジェクトでも実際に確認・修正済み）。なお HW ウォッチドッグが有効な今は、
いずれの FAILED も前述の通り通常 MainFunction の次サイクルを待たずにリセットに
至るため、ステータスの遷移そのものをログで観測できる場面は限られます。
