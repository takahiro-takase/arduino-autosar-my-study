...
| BSW | WdgM | SWS_WdgM | Supervised Entity の Alive Supervision を管理。`WdgM_CheckpointReached()` の呼び出し回数を 6000ms ごとに評価し、期待回数未達の場合はソフトウェア異常として WARN ログを出力 |
...
| BSW | Dem | SWS_Dem | 診断イベントを DTC として管理。NvM 経由で EEPROM に永続化。FAILED 遷移時に FreezeFrame（RAM のみ）を記録 |
...
│       ├── WdgM/                 # ウォッチドッグマネージャ（Alive Supervision）
│       │   ├── WdgM_Cfg.h        # エンティティ ID・監視サイクル・期待回数定義
│       │   ├── WdgM_PBCfg.h      # エンティティ設定構造体型定義・WdgM_Config 宣言
│       │   ├── WdgM_PBCfg.c      # エンティティテーブル実体（App_EngineManager_Run）
│       │   ├── WdgM.h            # 公開インタフェース（Init / CheckpointReached / MainFunction）
│       │   └── WdgM.c            # Alive カウンタ管理・6000ms ごとの評価ロジック
...
| SID | サービス | SubFunc | フレーム例（byte0=PCI） | 可変バイト |
|---|---|---|---|---|
| 0x10 | DiagnosticSessionControl | 0x01 (Default) | `02 10 01 00 00 00 00 00` | — |
| 0x10 | DiagnosticSessionControl | 0x03 (Extended) | `02 10 03 00 00 00 00 00` | — |
| 0x11 | ECUReset | 0x01 (hardReset) | `02 11 01 00 00 00 00 00` | — |
| 0x11 | ECUReset | 0x03 (softReset) | `02 11 03 00 00 00 00 00` | — |
| 0x14 | ClearDiagnosticInformation | — | `04 14 FF FF FF 00 00 00` | byte2-4=groupOfDTC（0xFFFFFF 固定） |
| 0x19 | ReadDTCInformation | 0x01 (件数取得) | `03 19 01 MM 00 00 00 00` | byte3=statusMask |
| 0x19 | ReadDTCInformation | 0x02 (DTC 一覧取得) | `03 19 02 MM 00 00 00 00` | byte3=statusMask |
| 0x19 | ReadDTCInformation | 0x04 (FreezeFrame取得) | `06 19 04 HH MM LL RR 00` | byte3-5=DTC コード, byte6=recordNumber（固定 0x01） |
| 0x22 | ReadDataByIdentifier | — | `03 22 HH LL 00 00 00 00` | byte2-3=DID（0x0101 / 0x0102 / 0x0103） |
| 0x3E | TesterPresent | 0x00 | `02 3E 00 00 00 00 00 00` | — |
...
WdgM は監視対象（Supervised Entity）に「生存報告」を埋め込み、報告が途絶えたときに異常と判断します。
...
監視対象 Runnable が呼ぶ:
  App_EngineManager_Run()  (3000ms 周期)
    └→ WdgM_CheckpointReached(WDGM_ENTITY_ENGINE)  ← "私は正常に完了した"

WdgM_MainFunction (6000ms 周期) が評価:
  AliveCount >= WDGM_EXPECTED_ALIVE_INDICATIONS (1) ?
    YES → LOCAL_STATUS_OK  → "SE0 OK alive=2"
...
**正常時（6000ms ごと）:**
```
[6017ms]  DEBUG WdgM: SE0 OK alive=2   ← 3000ms 周期で 2 回 CheckpointReached が来た
[12017ms] DEBUG WdgM: SE0 OK alive=2
```

**失敗検知（POST_RUN 移行後に Rte_Engine タスクが停止した 6 秒後）:**
```
[30312ms] INFO  EcuM: ->POST_RUN timeout=5000ms
[36312ms] WARN  WdgM: SE0 FAILED alive=0 (exp>=1) [HW reset in production]
```

**動作確認方法（`WdgM_CheckpointReached` をコメントアウト）:**
```c
/* (void)WdgM_CheckpointReached(WDGM_ENTITY_ENGINE); */
```
起動後 6000ms で FAILED ログが出る。コメントを戻すと次のサイクルで自動回復。
...
確定押下が継続するたびに (IoHwAb_MainFunction):
  ...
  s_stuckCounter == 500?
    ...
    → WARN: "Button stuck dtc=0x000106"      ← DTC 0x000106 が EEPROM に保存

ボタン解放時:
  if s_stuckCounter >= 500:
    → Dem_ReportErrorStatus(DEM_EVENT_BUTTON_STUCK, PASSED)
    → INFO: "Button stuck cleared"
  s_stuckCounter = 0
```
...
# ボタンを 5 秒以上押しっぱなし → 固着検出 → DTC 0x000106 記録
...
# ADC センサ電圧が 1000mV 未満に低下 → DTC 0x000107 記録
[12010ms] DEBUG AppEng: ADC=820mV                # 閾値未満（10ms 周期の IoHwAb_MainFunction が検出）
[12011ms] WARN  Dem: FAILED ev=6 dtc=0x000107     # EEPROM に保存
# 電圧が復帰すると PASSED 報告（TF ビットクリア・CDTC は残る）
[15010ms] DEBUG AppEng: ADC=3300mV
[15011ms] INFO  Dem: PASSED ev=6
...
[9012ms] INFO  Dem: FreezeFrame ev=0 spd=1000 tmp=101 st=2  # 故障時のスナップショット（RAM のみ）

# UDS SID 0x19/04: 上記 ENGINE_OVERHEAT のFreezeFrame取得
[9500ms] INFO  CanTp: RX SF len=6
[9501ms] INFO  Dcm: req SID=0x19
[9502ms] INFO  Dcm: 19/04 dtc=0x000101 rec=1
[9503ms] INFO  CanTp: TX FF len=18        # 18 バイト → FF+CF×2 に分割
[9504ms] DEBUG Can_Hw: TX OK id=0x7E8 dlc=8 [10 12 59 04 00 01 01 2D]
