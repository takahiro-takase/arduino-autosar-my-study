...
- **TX（CAN ID 0x220）**: E2E 検証エラーの累積カウンタ（CRC不一致/シーケンス異常）を Com の PERIODIC 送信モードで 1000ms 周期送信（E2EMon、独自 CDD 相当、AUTOSAR E2E Profile 01 保護付き）
...
|  | Com | SWS_Com | シグナルのビット単位パック／アンパックと受信デッドライン監視（タイムアウト検出）のみを担い、E2E には一切関知しない（E2E Transformer 方式）。I-PDU ごとの `RxIndicationCbk`/`TxTransformCbk`（`Com_IPduConfigType` の汎用フック、`Com_PBCfg.c` で設定）を通じて Rte 層のグルー関数を呼ぶだけで、中身が E2E であることも Com.c 本体には埋め込まれない。TX I-PDU ごとに `TxModeMode`（MIXED/PERIODIC）を設定でき、MIXED は ComFilterAlgorithm により「送信すべきか」を Com が判断（周期フロアあり、ASW が `Com_TriggerIPDUSend()` を呼んだ時のみ評価）、PERIODIC は Com 自身が `Com_MainFunction()` の中で実時間ベースに送信タイミングを判断し、ASW/CDD は一切関与しない。WarningStatus (0x210) は Signal Group として複数シグナルを `Com_SendSignalGroup()` でシャドウバッファから一括コミットする |
...
**MeterStatus（メータ ECU / CAN ID 0x200 / DLC=1 / E2E 保護なし）**

`App_EngineManager_Run()`（3000ms 周期）が毎サイクル送信を要求しますが、実際に CAN へ
出るのは `EngineState` が変化したとき、または ComFilterAlgorithm の周期フロア（後述）に
達したときだけです（詳細は次項）。E2E 保護は付与していません（EngineInfo/AbsInfo を
Com が既に検証した**後**にメータ ECU 自身が導出する二次データであり、実車でも一次
センサ値ほど厳密な保護が付与されないことが多いため、素の（E2E 保護なしの）シグナル
送信の実装例として意図的に残しています。詳細は「E2E P01 保護」セクション参照）。
...
集計し、Com の PERIODIC 送信モードにより 1000ms 周期で自動送信されるネットワーク
...
長さ付きで呼び出すだけです（MIXED モードの `Com_TriggerIPDUSend()` と同じ
「送信直前の最終変換」の仕組みをそのまま再利用しています）。実際に Counter・
...
`ComTxModeMode` 相当の `TxModeMode` 設定（`COM_TX_MODE_MIXED` / `COM_TX_MODE_PERIODIC`）
を追加し、`E2EHealthStatus` は `COM_TX_MODE_PERIODIC` として `Com_MainFunction()`
が自分の周期タイマ（既定 1000ms、`COM_TX_PERIOD_E2EHEALTH_MS`）で自動送信します。
...
カウンタ飛びを起こすと byte[3]（seqErr）が増えます。1000ms 周期で自動送信される
ため、値が変化していなくても定期的にフレームが流れ続けることも確認できます。

```
[1019ms] INFO  Com: TX iPdu=2 [XX 00 00 00]  # E2EHealthStatus、1000ms 周期で自動送信
[2019ms] INFO  Com: TX iPdu=2 [YY 01 00 00]  # Counter が 1 に進む
...
[9019ms] INFO  Com: TX iPdu=2 [ZZ 02 01 00]  # crcErr が 0→1 に増加
```
...
| Com TX I-PDU の送信モード変更（MIXED/PERIODIC）・PERIODIC周期変更 | `Com_PBCfg.c` の該当 IPdu の `TxModeMode`/`TxPeriodMs`、周期定数は `Com_Cfg.h` |
