...
|  | WdgM | SWS_WdgM | Supervised Entity の Alive Supervision（呼び出し回数を6000msごとに評価）と Logical Supervision（チェックポイント順序を即時検証）を管理。異常時はソフトウェア異常として WARN ログを出力 |
|  | ComM | SWS_ComM | CAN バスの通信モード（NO_COM / FULL_COM）を管理し CanSM へ要求。`ComM_BusSMIndication` で EcuM の RUN 要求を操作 |
|  | CanSM | SWS_CanSM | Bus-Off 発生時の回復シーケンス（内側 3 回×外側 2 回の 2 段リトライ）を実施。回復断念・成功時に `ComM_BusSMIndication` を呼ぶ。回復断念は Dem にも DTC として報告（外側 2 回目でデバウンス確定） |
...
|  | Dcm | SWS_Dcm | UDS 診断サービス処理（SID 0x10 / 0x11 / 0x14 / 0x19 / 0x22 / 0x3E）。S3 タイマでセッションを自動失効 |
|  | Dem | SWS_Dem | 診断イベントを DTC として管理。カウンタベースのデバウンスで確定し、NvM 経由で EEPROM に永続化。デバウンス確定 FAILED 時に FreezeFrame（RAM のみ）を記録 |
...
│       │   └── WdgM.c            # Alive カウンタ管理・チェックポイント順序検証・6000ms ごとの評価ロジック
...
固定バイト・可変バイトと正応答 SID をまとめます。byte0 は CanTp の SF PCI（UDS ペイロード長）です。
全サービスの要求ペイロードは 7 バイト以内のため、要求は必ず SF で送信できます
（応答が長くなるケースは個別に後述します）。

| SID<br>サービス名 | SubFunc | 要求フレーム（byte0=PCI） | 可変バイト | 正応答 SID |
|---|---|---|---|---|
| 0x10<br>DiagnosticSessionControl | 0x01<br>(Default) | `02 10 01 00 00 00 00 00` | — | 0x50 |
|  | 0x03<br>(Extended) | `02 10 03 00 00 00 00 00` | — | 0x50（S3タイマ起動） |
| 0x11<br>ECUReset | 0x01<br>(hardReset) | `02 11 01 00 00 00 00 00` | — | 0x51 |
|  | 0x03<br>(softReset) | `02 11 03 00 00 00 00 00` | — | 0x51 |
| 0x14<br>ClearDiagnosticInformation | — | `04 14 FF FF FF 00 00 00` | byte2-4=groupOfDTC<br>・0xFFFFFF=全クリア<br>・DTCコード指定=1件クリア | 0x54 |
| 0x19<br>ReadDTCInformation | 0x01<br>(件数取得) | `03 19 01 MM 00 00 00 00` | byte3=statusMask | 0x59 |
|  | 0x02<br>(DTC一覧取得) | `03 19 02 MM 00 00 00 00` | byte3=statusMask | 0x59 |
|  | 0x04<br>(FreezeFrame取得) | `06 19 04 HH MM LL RR 00` | byte3-5=DTCコード<br>byte6=recordNumber（固定0x01） | 0x59 |
| 0x22<br>ReadDataByIdentifier | — | `03 22 HH LL 00 00 00 00` | byte2-3=DID（0x0101/0x0102/0x0103） | 0x62 |
| 0x3E<br>TesterPresent | 0x00 | `02 3E 00 00 00 00 00 00` | — | 0x7E（S3タイマ維持） |
...
各イベントは `DEM_DEBOUNCE_LIMIT`（既定 2）を上下限とするデバウンスカウンタを持ちます。
FAILED 報告でカウンタ +1、PASSED 報告で -1 し（上下限でクランプ）、カウンタが
**±LIMIT に達した瞬間にのみ** DTC ステータス（TF/PDTC/CDTC/TFSLC）を確定します。
1 回だけの報告では確定せず PRE-FAILED / PRE-PASSED の中間状態に留まるため、
単発のノイズや一時的な誤検出だけで DTC が確定することを防ぎます。

```
FAILED 報告  → counter++  (上限 +LIMIT でクランプ)
PASSED 報告  → counter--  (下限 -LIMIT でクランプ)
...
イベントごとの報告パターンによって、確定までにかかる実時間が大きく異なります。

| イベント | 報告パターン | 確定までの目安 |
|---|---|---|
| ENGINE_OVERHEAT / STALL / SPEED_NO_FLAG / STARTING_TIMEOUT | 状態遷移の瞬間に単発報告 | 同じ故障が**別々の機会に 2 回**発生する必要あり |
| COMM_TIMEOUT | 故障継続中は毎 Runnable サイクル（3000ms）報告 | 2 サイクル分＝約 3000〜6000ms 追加 |
| BUTTON_STUCK / ADC_VOLT_LOW | 故障継続中は毎 10ms サイクル報告 | 数十 ms（実質的に即時） |

> IoHwAb の BUTTON_STUCK は、5000ms の物理固着判定後は毎サイクル FAILED を Dem へ報告するよう変更しています。
> 以前のように固着検出時に 1 回だけ報告する実装では、解放時の PASSED 1 回（counter-1）と
> 相殺し合って確定（counter+2 が必要）に決して到達できない問題があったためです。
...
Arduino UNO の内蔵 EEPROM 先頭 9 バイトを使用します。
...
タスク数が 8 を超えるため、`TaskMask` は uint16（bits 0〜8）です。
...
POST_RUN ではこの 2 タスクだけを停止し、BSW タスク（CAN ISR・CanTp・CanSM・Com・IoHwAb・WdgM・Dcm）は継続させます。
...
POST_RUN 中も動き続けるタスク:
  ...
  WdgM_Main          → Alive Supervision を最後のサイクルまで継続
...
WdgM_MainFunction (6000ms 周期) が評価:
  AliveCount >= WDGM_EXPECTED_ALIVE_INDICATIONS (1) ?
    ...
    NO  → LOCAL_STATUS_FAILED → "SE0 FAILED alive=0 [HW reset in production]"
...
| 環境 | 失敗時のアクション |
|---|---|
| 本プロジェクト（Arduino UNO） | WARN ログ出力のみ（HW ウォッチドッグ未搭載） |
| 実機製品 | WdgM が HW ウォッチドッグのリフレッシュを停止 → タイムアウト後にシステムリセット |
...
**Alive 失敗検知（POST_RUN 移行後に Rte_Engine タスクが停止した 6 秒後）:**
```
[30312ms] INFO  EcuM: ->POST_RUN timeout=5000ms
[36312ms] WARN  WdgM: SE0 FAILED alive=0 (exp>=1) [HW reset in production]
```

**Logical 失敗検知（後述の動作確認方法で START 呼び出しを止めた場合）:**
```
[3010ms] WARN  WdgM: SE0 logical FAILED cp 1->1 (unexpected) [HW reset in production]
                                  └┘  └┘
...
```
起動後 6000ms で `alive=0` の FAILED ログが出る。コメントを戻すと次のサイクルで自動回復。
...
確定押下状態（`s_confirmedLevel == 1`）が 5000ms（= 500 × 10ms）継続すると Dem への報告を開始します。
しきい値到達後は毎サイクル FAILED を、解放後は毎サイクル PASSED を Dem へ報告し、
DTC を確定するかどうかの判断は Dem 側のデバウンスカウンタ（前述）に委ねます。

```
確定押下が継続するたびに (IoHwAb_MainFunction):
  s_stuckCounter++ (上限 500 でクランプ)
  s_stuckCounter == 500 になった瞬間 → WARN: "Button stuck dtc=0x000106"（検出ログ、1 回のみ）
  s_stuckCounter >= 500 の間は毎サイクル → Dem_ReportErrorStatus(DEM_EVENT_BUTTON_STUCK, FAILED)
    → Dem のデバウンスカウンタが 2 に達した時点で DTC 確定・EEPROM 保存

ボタン解放時:
  s_stuckCounter >= 500 だった場合 → INFO: "Button stuck cleared"（1 回のみ）
  s_stuckCounter = 0
  毎サイクル → Dem_ReportErrorStatus(DEM_EVENT_BUTTON_STUCK, PASSED)
    → Dem のデバウンスカウンタが -2 に達した時点で TF クリア（CDTC は残る）
```

固着検出後は 10ms 周期で毎回報告されるため、Dem 側のデバウンス確定（カウンタ 2 回分）は
数十 ms 以内に完了します。固着検出の遅延（5000ms）に比べて無視できる差です。
...
| LED | ピン | 点灯条件 | 制御 API |
|-----|------|---------|---------|
| RUNNING 灯 | D6 | `EngineState == RUNNING` | `IoHwAb_LedRunning_SetLevel` |
...
# ボタンを 5 秒以上押しっぱなし → 固着検出 → 毎サイクル報告で 2 回目にデバウンス確定 → DTC 0x000106 記録
[1000ms] INFO  IoHwAb: Button confirmed level=1  # 押下確定（40ms 後）
[6000ms] WARN  IoHwAb: Button stuck dtc=0x000106 # 5000ms 経過で固着判定（検出ログ、1 回のみ）
[6000ms] DEBUG Dem: ev=5 debounce=1 (PREFAILED)  # 1 回目の FAILED 報告（まだ未確定）
[6010ms] WARN  Dem: FAILED ev=5 dtc=0x000106     # 2 回目（10ms 後）でデバウンス確定・EEPROM に保存
# ボタン解放 → 毎サイクル PASSED 報告 → 2 回目でデバウンス確定（TF ビットクリア・CDTC は残る）
[8000ms] INFO  IoHwAb: Button confirmed level=0
[8000ms] INFO  IoHwAb: Button stuck cleared
[8000ms] DEBUG Dem: ev=5 debounce=-1 (PREPASSED)
[8010ms] INFO  Dem: PASSED ev=5                  # testFailed クリア
...
[15313ms] DEBUG Dem: ev=7 debounce=-1 (PREPASSED)  # 回復成功ごとに PASSED 報告（単発では未確定）
[15314ms] INFO  ComM: ch0 ->mode=2        # ComM_BusSMIndication(FULL_COM) → EcuM_RequestRUN → RUN 維持
# → TX 成功 → 通常動作に復帰

# アダプタ未接続のままなら、内側リトライ（3 回）を使い切るたびに「1 回分の断念」を記録する
# 1 回目の内側リトライ枯渇（約 800ms 後）→ まだ最終断念ではない（外側 1/2）
[15912ms] ERROR CanSM: BusOff: inner retries (3) exhausted, giveup 1/2
[15913ms] DEBUG Dem: ev=7 debounce=1 (PREFAILED)   # 1 回目の FAILED 報告（まだ確定しない）
# 内側リトライカウンタをリセットして回復サイクルを再開（アダプタ未接続のため再び枯渇する）
[16712ms] ERROR CanSM: BusOff: inner retries (3) exhausted, giveup 2/2
[16713ms] WARN  Dem: FAILED ev=7 dtc=0x000108       # 2 回目でデバウンス確定・EEPROM に保存
[16714ms] INFO  Dem: FreezeFrame ev=7 spd=... tmp=... st=...  # 断念時点のスナップショット
[16715ms] ERROR CanSM: BusOff: giveup limit (2) reached, recovery stopped
[16716ms] INFO  ComM: ch0 ->mode=0        # ComM_BusSMIndication(NO_COM) → EcuM_ReleaseRUN
[16716ms] INFO  EcuM: ->POST_RUN timeout=5000ms
[21716ms] INFO  EcuM: ->SHUTDOWN          # スケジューラ停止（タスク実行なし）
...
| Dem デバウンス閾値の変更 | `Dem_Cfg.h` の `DEM_DEBOUNCE_LIMIT` |
...
| CanSM Bus-Off リトライ回数（内側/外側）の変更 | `CanSM_Cfg.h` の `CANSM_BUSOFF_MAX_RETRIES` / `CANSM_BUSOFF_MAX_GIVEUP_CYCLES` |
