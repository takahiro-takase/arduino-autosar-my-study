...
無条件に即座に送信する」ことを要求します（SWS_Com_00495）。本実装では
別途この処理を実装せず、既存の「Signal Group 全体のバイト比較による変化検知」
（`Com_SendSignalGroup()` の `changed` 判定）にそのまま乗せています。TMS 寄与
シグナルのビットは常にこの比較対象バイトに含まれるため、TMS が変化する場面は
必ず `changed=1` にもなり、`Com_RequestTxOnChange()` が呼ばれます。つまり
「バイト全体が変化 ⊇ TMS 対象ビットが変化」という包含関係により、追加コード
なしで SWS_Com_00495 相当の即時送信が成立しています。
