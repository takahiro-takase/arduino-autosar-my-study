...
- **警告灯 LED（D8）**: FAULT=点滅（最優先）/ ABS 作動=点灯 / RUNNING=点灯 / その他=消灯
...
│       App_WarningIndicator 警告灯 LED 優先度制御       │
├──────────────────────────────────────────────────────┤
...
│       Dio_Hw  Arduino digitalWrite ラッパー            │  C++ のみ
...
│   │   └── App_WarningIndicator.c   # 警告灯 LED 優先度制御（FAULT=点滅 > ABS=点灯 > RUNNING=点灯）
│   ├── Os/
│   │   ├── Os_Cfg.h              # タスク数定数
...
│       │   ├── Dio_Cfg.h         # チャネル ID 定義（DIO_CHANNEL_LED_WARNING = D8）
│       │   ├── Dio.h             # 公開インタフェース（Dio_WriteChannel のみ）
│       │   ├── Dio.c             # AUTOSAR Dio モジュール（純粋 C、Dio_Hw へ委譲）
│       │   ├── Dio_Hw.h          # 内部インタフェース（Dio.c と Dio_Hw.cpp の境界）
│       │   └── Dio_Hw.cpp        # Arduino digitalWrite ラッパー（C++ のみ）
│       ├── Port/                 # ピン方向設定（MCAL・Dio と責務を分離）
│       │   ├── Port_Cfg.h        # ピン番号定義（PORT_PIN_LED_WARNING = D8）
...
│       │   └── IoHwAb.c          # Dio_WriteChannel へ委譲（Dio_Cfg.h を知る唯一の非 MCAL ファイル）
...
| 送信フレーム | AbsActive | BrakeActive | LED 期待動作 |
|------------|-----------|-------------|------------|
| `110#27.10.00` | 0 | 0 | ON（RUNNING のみ） |
| `110#27.10.40` | 0 | 1 | ON（BrakeActive は LED に影響しない） |
| `110#27.10.C0` | 1 | 1 | **ON(ABS)**（ABS 警告） |
| `110#27.10.80` | 1 | 0 | **ON(ABS)**（ABS 警告） |

FAULT 状態で `110#27.10.C0`（AbsActive=1）を送信しても、LED は BLINK(Fault) を維持します（優先度確認）。
...
| LED + 抵抗（220〜470 Ω）| 警告灯インジケータ（D8 に接続） |
...
> 警告灯 LED は D8（`DIO_CHANNEL_LED_WARNING`）に 220〜470 Ω の抵抗を直列に挿入して接続してください。
...
[2ms] INFO  Port: Init pins=1             # ピン方向設定（D8 を OUTPUT に）
...
[17ms] INFO  Os: Init ok tasks=6          # タスクスケジューラ起動（全モジュール初期化後）
...
[6500ms] INFO  WarnInd: LED ON            # RUNNING → LED 点灯

# 0x110 受信時（AbsActive=1）→ LED ON(ABS)
[7000ms] DEBUG Can_Hw: RX OK id=0x110 dlc=3 [27 10 C0]
[7001ms] DEBUG Com: RX iPdu=1 [27 10 C0]
[7500ms] INFO  WarnInd: LED ON(ABS)       # AbsActive=1 → ABS 警告点灯
...
[6500ms] INFO  WarnInd: LED BLINK(Fault)

# 過熱検出（CoolantTemp=101℃）→ RUNNING→FAULT（ABS 同時作動中でも FAULT が最優先）
[9010ms] WARN  AppEng: RUNNING->FAULT overheat=101
[9500ms] INFO  WarnInd: LED BLINK(Fault)  # FAULT → 500ms ごとにトグル（ABS より優先）
[10000ms] INFO  WarnInd: LED BLINK(Fault)
...
[15112ms] INFO  WarnInd: LED OFF           # EngineState が不定に
...
| 警告灯 LED のピン番号変更 | `Dio_Cfg.h`（`DIO_CHANNEL_LED_WARNING`） |
