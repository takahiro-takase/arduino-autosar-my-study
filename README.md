...
|  | CanSM | SWS_CanSM | Bus-Off 発生時の回復シーケンス（最大 3 回リトライ）を実施。回復断念・成功時に `ComM_BusSMIndication` を呼ぶ。回復断念は Dem にも DTC として報告 |
...
[30313ms] DEBUG Dem: ev=7 debounce=1 (PREFAILED)   # 1 回目（別の機会の Bus-Off 断念で 2 回目に確定）
[30314ms] INFO  ComM: ch0 ->mode=0        # ComM_BusSMIndication(NO_COM) → EcuM_ReleaseRUN
[30312ms] INFO  EcuM: ->POST_RUN timeout=5000ms
