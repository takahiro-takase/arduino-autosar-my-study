...
（このシグナル専用の代替値）で置き換える（SUBSTITUTE、SWS_Com_00875）の 3 択です。
...
ありません。したがって SUBSTITUTE は「呼び出せば正しく動く」ことをロジックとして
確認できる状態ではありますが、`TMS`/`MDT`/`ComTransferProperty`/RX Signal Group と
同じく、動機は実利より仕様忠実性です。これが実際に意味を持つのは、ASW/RTE が
E2E Transformer のような中間ミラーを介さず `Com_ReceiveSignal()` を直接、
かつフレーム受信タイミングとは無関係な任意のタイミングで呼ぶような構成です。
