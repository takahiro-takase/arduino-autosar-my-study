...
`test/Bsw_ComStack_RxTimeoutChain_test.cpp` の
...
**この機能は実際に発動するか**: **受理側は実機で確認済みです**。
`uds_tester` の「ImmobilizerCmd鍵('1') → 新鍵」プリセット等で正規の
SecOC 鍵付き `ImmobilizerCmd=UNLOCK(0x01)` を送ると、ログで
`Com_GatewayRoute: Gateway src=12 -> dst=13 value=1` の直後に
`Com_DoTransmit: TX iPdu=3 [01]` が出力され、CAN 0x230 (`ImmobilizerStatus`)
へ実送信されることを確認しました（`TxIpduCalloutCbk` が `0x01` を正しく
受理している）。`NumberOfRepetitions=2` による計3回の再送（100ms間隔）も
すべて正しく受理・送信されており、既存の再送機構との相互作用に問題が
ないことも合わせて確認できました。
