...
```
（修正前）
Com_MainFunctionTx() → Com_DoTransmit() → PduR_ComTransmit() → CanIf_Transmit() → Can_Write()
...
非同期書き込みジョブキュー（[NvM_Notes.md参照](./NvM_Notes.md#非同期書き込みジョブキューへの変更経緯)）
