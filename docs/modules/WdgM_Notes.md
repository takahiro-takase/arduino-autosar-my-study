...
> 後述の「[開発の経緯](#deadline-supervision-上限緩和と-os_schedulerstep-のバグ)」を参照。
...
後述の「[開発の経緯](#グローバル猶予カウンタを-resume-でリセットしてはいけなかった)」参照）。
...
後述の「開発の経緯」の
[POST_RUN→RUN 復帰時の Deadline Supervision 誤検出](#post_runrun-復帰時の-deadline-supervision-誤検出)、
[Alive Supervision 誤検出](#post_runrun-復帰時の-alive-supervision-誤検出)、
[短時間 POST_RUN での誤検出](#短時間-post_run-での-alive-supervision-誤検出)
を参照してください。
...
後述の「[開発の経緯](#alive-と-logical-のステータス統合バグ)」参照）。
...
### Deadline Supervision 上限緩和と Os_SchedulerStep のバグ
...
### POST_RUN→RUN 復帰時の Deadline Supervision 誤検出
...
### POST_RUN→RUN 復帰時の Alive Supervision 誤検出
...
### 短時間 POST_RUN での Alive Supervision 誤検出
