 ...
 * \details Supervised Entity の Alive Supervision インタフェース。
 *
 *          使い方:
 *            1. EcuM_Init 内で WdgM_Init(&WdgM_Config) を呼ぶ。
 *            2. 監視対象の Runnable 末尾で WdgM_CheckpointReached(WDGM_ENTITY_*) を呼ぶ。
 *            3. Os スケジューラが WdgM_MainFunction() を定期実行する。
 *               CheckpointReached が WDGM_EXPECTED_ALIVE_INDICATIONS 回以上来ていれば OK、
 *               0 回のまま周期が来たら FAILED とみなしログで通知する。
 *
 *          本プロジェクトでの失敗アクション:
 ...
 *
 * \details 監視対象の Runnable 末尾から呼び出す。
 *          WdgM 内部の Alive カウンタをインクリメントする。
 *
