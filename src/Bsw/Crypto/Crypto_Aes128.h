/**
 * \file    Crypto_Aes128.h
 * \brief   AES-128 単一ブロック暗号 (FIPS-197 準拠)
 * \details Crypto Driver が MAC 生成/検証プリミティブ（Crypto_Cmac.c）の下位で
 *          使う暗号ブロックプリミティブ。暗号化のみを提供し、復号は実装しない
 *          （CMAC は暗号化のみで構成できる）。外部ライブラリに依存しない自前実装。
 *
 *          元は SecOC モジュール内に直接持っていた実装（SecOC_Aes128.c）を、
 *          Csm/CryIf/Crypto レイヤ分離の一環として Crypto Driver 側へ移設した
 *          （ロジックは無変更。SecOC は本ファイルの存在を一切知らない）。
 *
...
#define CRYPTO_AES128_BLOCK_SIZE  16U  /* AES のブロック長は鍵長によらず常に 128bit = 16byte */
#define CRYPTO_AES128_KEY_SIZE    16U  /* AES-128 の鍵長 */
 ...
 *          利用モードは呼び出し側（Crypto_Cmac）の責務とする。
 *
 ...
 *          `Crypto_Init()` から起動時セルフテストとして呼ばれる。
 *
