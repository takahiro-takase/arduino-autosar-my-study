 ...
 * \details CanNm が送受信する NM PDU のパラメータと、CanNm 状態機械のタイマ値を
 *          定義する。
 *
 *          本プロジェクトでの位置づけ:
 ...
 *
 * 2026-08-30 追記: CanNm_Init/NetworkRequest/NetworkRelease/RepeatMessageRequest/
 * GetState は当初 NetworkHandle 引数を一切取らない設計だったが、IF
 * シグネチャは仕様準拠という方針のもと NetworkHandleType 引数を追加した
 * （CanNm.h 各関数の doc コメント参照）。単一チャネル固定の本プロジェクトでも、
 * 同じく単一チャネル固定の CanSM が CANSM_E_INVALID_NETWORK_HANDLE で
 * 範囲チェックを行っている（CanSM.c 参照）のと平仄を合わせ、CANNM_E_INVALID_
...
#define CANNM_E_UNINIT           0x01U  /* [SWS_CanNm_00002/00191]: 未初期化時の API 呼び出し */
#define CANNM_E_INVALID_CHANNEL  0x02U  /* [SWS_CanNm_00192]: NetworkHandle が
                                      *  CANNM_MAIN_NETWORK_HANDLE 以外 */
#define CANNM_E_NET_START_IND    0x04U  /* [SWS_CanNm_00336]: Bus-Sleep Mode 中に NM PDU を受信 */
#define CANNM_E_NETWORK_TIMEOUT  0x11U  /* [SWS_CanNm_00193/00194]: Repeat Message/Normal
                                      *  Operation State で NM-Timeout Timer が満了 */
#define CANNM_E_PARAM_POINTER    0x12U  /* NULL ポインタチェック */
...
#define CANNM_API_ID_NETWORK_REQUEST          0x02U
#define CANNM_API_ID_NETWORK_RELEASE          0x03U
#define CANNM_API_ID_REPEAT_MESSAGE_REQUEST   0x08U
#define CANNM_API_ID_GET_STATE                0x0BU
/** 2026-09、独自の `CanNm_SetTxEnabled(uint8 Enabled)`（1つの bool 引数で
...
#define CANNM_API_ID_GET_VERSION_INFO         0xF1U

/** バージョン情報（CanNm_GetVersionInfo、Com/E2EXf/PduR 等の既存モジュールと同じ命名規則） */
#define CANNM_VENDOR_ID          0U
#define CANNM_SW_MAJOR_VERSION   1U
...
/** CanNm_MainFunction() の呼び出し周期 [ms]（Os_PBCfg.c のタスク周期と一致させること）。
...
#define CANNM_SOURCE_NODE_ID  0x01U

/** 本プロジェクトが持つ唯一の NM チャネルのハンドル値。呼び出し元はこの値を
 *  渡すこと。これ以外の値は CANNM_E_INVALID_CHANNEL として拒否される
 *  （[SWS_CanNm_00192]、CanNm_NetworkRequest() 等参照）。 */
#define CANNM_MAIN_NETWORK_HANDLE  0U
...
#define CANNM_CBV_BIT_REPEAT_MESSAGE_REQUEST  0x01U

/**
 * CanIf に登録した本 NM フレームの TxPduId。
 * CanIf_PBCfg.c の CanIf_TxPduConfigData 配列インデックスと一致させること
 * （CanNm は PduR/Com を経由せず CanIf_Transmit() を直接呼ぶため、
 * 実車の CanNm と同様に Com スタックとは独立して動作する）。
 */
#define CANNM_CANIF_TX_PDU_ID  2U
 ...
 *  値の選定は任意。CANNM_CYCLE_MS の周期で少なくとも1回は正常送信/受信できる
