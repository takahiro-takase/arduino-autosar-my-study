 ...
 *
 *          WdgM_ENTITY_WARNING (WdgM_Cfg.h) の Supervised Entity として
 *          Runnable 開始時に WDGM_CP_WARNING_START、終了時に WDGM_CP_WARNING_END
 *          を報告する（App_EngineManager_Run と同じ Alive/Logical/Deadline
 *          Supervision の対象）。
 *
...
void App_WarningIndicator_Run(void)
{
    ...
    (void)WdgM_CheckpointReached(WDGM_ENTITY_WARNING, WDGM_CP_WARNING_START);
    ...
    /* WarningStatus (CAN 0x210): 3 本の警告灯状態を Signal Group として
     * まとめて他 ECU へ配信する。FAULT LED の点滅により faultLevel が
     * 500ms ごとに変化するため、点滅中は毎回コミットされ送信される。 */
    (void)Rte_Write_WarningStatus_RunLamp(runLevel);
