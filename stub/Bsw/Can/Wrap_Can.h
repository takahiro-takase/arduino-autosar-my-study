 ...
 *          （既定 `WRAP_CAN_FAIL_FROM_CALL_COUNT_DISABLED`）に N を設定すると、
...
#define WRAP_CAN_FAIL_FROM_CALL_COUNT_DISABLED 0xFFFFFFFFU

/* ----------------------------------------------------------------------
 * 呼び出し回数（AUTOSAR IF 全11関数共通）
 * ---------------------------------------------------------------------- */
extern uint32 CallCount_Can_Init;
extern uint32 CallCount_Can_GetVersionInfo;
extern uint32 CallCount_Can_SetControllerMode;
extern uint32 CallCount_Can_GetControllerErrorState;
extern uint32 CallCount_Can_DisableControllerInterrupts;
extern uint32 CallCount_Can_EnableControllerInterrupts;
extern uint32 CallCount_Can_Write;
extern uint32 CallCount_Can_MainFunctionRead;
extern uint32 CallCount_Can_MainFunctionWrite;
extern uint32 CallCount_Can_MainFunctionBusOff;
extern uint32 CallCount_Can_MainFunctionWakeup;

/* ----------------------------------------------------------------------
 * 回数閾値故障注入（戻り値を持つ3関数のみ）
 * ---------------------------------------------------------------------- */
/** `WRAP_CAN_FAIL_FROM_CALL_COUNT_DISABLED`（既定）: 常に対応する
 *  `__real_Can_Xxx()` へパススルー。それ以外の値を設定すると、対応する
 *  `CallCount_Can_Xxx` がこの値に達した回から（以降ずっと）
 *  対応する `ForcedReturn_Can_Xxx` を返す。 */
extern uint32 FailFromCallCount_Can_SetControllerMode;

/** 0 以外を設定した場合、`ErrorStatePtr` には一切書き込まない。 */
extern uint32 FailFromCallCount_Can_GetControllerErrorState;

extern uint32 FailFromCallCount_Can_Write;

/* ----------------------------------------------------------------------
 * 閾値到達後の強制戻り値（戻り値を持つ3関数のみ）
 * ---------------------------------------------------------------------- */
extern Can_ReturnType ForcedReturn_Can_SetControllerMode;      /**< 既定 CAN_NOT_OK */
